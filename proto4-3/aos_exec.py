#!/usr/bin/env python3
"""aos-exec：單發執行器——把一個目標執行**一次**，回它的結束狀態。
...
def _run_inst(target, base, timeout_ms, on_spawn=None):
    ...
    if inst["exit"]:                    # 父目錄不存在＝aos-exec 自己失敗，不幫忙 mkdir
        ...
        if not os.path.isdir(d):
            return _err(1, "exit 檔的父目錄不存在（不會幫你建）：%s" % d)
    if not os.path.isdir(inst["cwd"]):  # chdir 失敗算設定階段失敗
        return _err(126, "cwd 不是資料夾：%s（exit 126）" % inst["cwd"])
...
def _finish(status, exit_path, note):
    """收尾：該說的說一句、該寫的 exit 檔寫掉，回那個結束狀態。"""
    if note:
        sys.stderr.write("aos-exec: %s\n" % note)
    if exit_path:
        try:
            ...
            return _err(1, "exit 檔寫不進去 %s：%s" % (exit_path, e))
    return status
...
def main(argv=None):
    ...
    return run_target(a.xxx, a.dir_target, a.timeout_ms)
