...
| 批的固定 `.temp` 槽位（roll-forward 的錨） | `/srv/demo/.aos/inst.json.temp` |
| 寫批用的每行程唯一暫存 | `/srv/demo/.aos/inst-<pid>-<seq>.json.temp` |
...
**唯一暫存名與固定槽位的分工**：兩者都存在，各解一個問題。

- **唯一名**（`inst-4711-0.json.temp`）解的是**併發**：權杖是 `<pid>-<seq>`，跟投遞
  用的是同一個計數器，所以兩個彙整者不可能共寫同一個檔。修補之前批 `.temp` 是固定名
  ＋ `O_TRUNC` ＋ 非排他，兩行程並寫會產生帶 NUL 空洞的混合內容，而破損的批會依
  §D-7 靜默消失＝資料遺失。
- **固定槽位**（`inst.json.temp`）解的是**崩潰**：它是 roll-forward 的錨。崩在提交點
  之後、批發布之前時，下一輪必須找得到那一批——而下一輪是另一個 pid，猜不到上一輪
  的唯一名，所以那一份必須躺在一個**約定好的**路徑上。

寫的順序因此是「唯一名寫完（含 fsync）→ 一次 `rename` 進固定槽位」。`rename` 是原子
的，所以固定槽位裡永遠是「某個行程寫完整的一份批」，不會再出現交錯寫的內容。

唯一名的形狀是把權杖插在最後一個 `.json` **之前**（`inst.json` → `inst-4711-0.json.temp`，
`inst-head.json` → `inst-head-4711-0.json.temp`）。這是刻意的：整體仍然符合 §B-1 的
`<名字>.<副檔名>.<狀況>` 文法，名字變成 `inst-4711-0`，沒有引進新的狀況字。隔離用的
第二個 `.bad` 名字（見下）走同一套規則。
...
```text
1. 寫批 <名字>-<pid>-<seq>.json.temp        → fsync 檔     ← 唯一名
2. rename → <base>.temp                                    ← 進固定槽位（原子）
3. 寫 header <名字>-head-<pid>-<seq>.json.temp → fsync 檔   ← 唯一名
4. rename header → <base>-head.json  ← 去重承諾的提交點（swept 寫 false）
5. fsync 目錄
6. rename 批：<base>.temp → base，**排他**                 ← 見下
7. fsync 目錄
8. 刪掉來源投遞 → fsync 目錄
9. 全數刪成功且目錄落盤 → 把 header 改寫成 swept:true
```

**批發布（第 6 步）必須排他**。目的檔已經存在就代表**別的彙整者先發布了**，本輪
MUST 放棄：不清投遞、不重寫 header、`published` 維持 `false`、回 `Ok`。修補之前這
一步是覆蓋語意的 `rename`，§D-4「`inst.json` 已有一份沒被讀走時本輪 MUST NOT 發布」
只靠開頭那個 `lstat` 擋——那是典型的 check-then-act，兩個彙整者同時跑就會互相覆蓋。

放棄時**固定槽位留著不刪**：那一份可能是別人的。它會在下一輪被覆蓋，而且只有在
「header 的 id 對得上 ＋ 位元組逐位元相同」時才會被 roll-forward，不會被誤扶正。

**排他發布的落敗者會留下一份「描述別人那批」的 header**（第 4 步已經跑完了）。這無害：
...
執行。只有 header 先 rename，才留得下「新 id ＋ 批在固定槽位」這個**可辨識、可
roll-forward** 的現場。
...
- 固定槽位的內容**與本輪重算的 canonical 位元組逐位元相同** → **roll forward**：
  排他 rename 到位（補完第 6 步）、`fsync` 目錄、清掉投遞。這一批不會丟。
- 位元組不同、或槽位讀不到 → 上一輪已經發布完、批也已經被取走或正在跑：
  **只清投遞、不重發**。這就是「同一批不會執行兩次」的兜底。
