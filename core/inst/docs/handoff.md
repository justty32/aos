...
排他發布只在勝出者那份批**還沒被取走**時有效。取件會把 `inst.json` rename 成
`.runi`，槽位就空了；慢半拍的彙整者這時 rename 過去照樣成功，於是同一批被發布第二
次。修補之前擋住這條路的其實是**永不失效的陳舊 header**——那正是 #1 的 bug 本體，
`swept` 閘門把它關掉之後，這個窗口就從「A 寫 header 之前」擴大成「A 標完 swept 之
後」，而後者沒有時間上限。實測：200 回合 ×2 支併發 exec，重複執行的回合比例由修補前
的 8.7% 升到 **35%**。

所以第 3 步之前多一道門（§D-4）：**發布之前再確認本輪收下的投遞都還在**，少了任何
一份就乾淨放棄——不發布、不清投遞、不寫 header。它擋得住這條路的主要窗口，因為
順序是固定的：**清投遞（第 7 步）必定發生在標 swept（第 8 步）之前**，所以等到閘門
開了、慢的那一支才可能走到這裡時，投遞早就不見了。加上這道門之後，同樣的實驗回到
**0–1%**（K=2／3／4 各跑三次），`LOST` 全部為 0。

剩下的那 0–1% 是查核與 rename 之間的窗口——那是「`.runi` 不是一把鎖」的本體，
**不在 M1 的射程內**（排 M2／M3，見
[gotchas](../../../wf/workflows/common/gotchas.md) 的「M1 審查交棒」節）。
這道門**不是互斥**，只是把窗口從「勝出者整批跑完」縮到幾個 syscall。
