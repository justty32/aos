...
一個指令檔就是可執行的權柄。它可以指名任意的程式、引數、輸入檔、輸出檔、工作目錄與環境變數值，而且全都使用呼叫者的憑證。執行一個不受信任的指令檔，等同於以你的權限執行任意命令。任何能修改 `aos inst` 將會讀取的檔案的人，實際上就擁有以該呼叫者身分執行任意程式碼的能力；請據此保護該檔的寫入權限與它的產生路徑。
