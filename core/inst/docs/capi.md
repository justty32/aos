...
**再 `fsync` 一次父目錄**，`fsync`、關檔或父目錄 `fsync` 失敗都算成失敗——寫入路徑
上的 `close()` 錯誤可能代表資料沒落地。父目錄那一步是承諾的一部分：只 `fsync` 檔案
本身保證不了**新建檔的目錄項**落盤，崩潰後可能得到「回了 `AOS_INST_OK`，檔案卻整個
不存在」。`AOS_INST_OK` 的意思是**內容與目錄項都落盤了**。
