...
extern "C" int aos_deliver_cli_main(int argc, char *argv[]) {
    const char *program = argc > 0 && argv != nullptr && argv[0] != nullptr
                              ? argv[0]
                              : "aos deliver";
    if (argv == nullptr || argc < 3 || argv[1] == nullptr ||
        argv[2] == nullptr) {
        usage(program);
        return 2;
    }
    ...
    if (std::strcmp(argv[2], "--") == 0) {
        ...
        command.reserve(static_cast<std::size_t>(argc - 3));
        ...
        std::string error;
        auto parsed = aos::wire::parse_inst(text, default_id(argv[2]), error);
        if (!parsed) {
    ...
    std::string error;
    const auto layout = aos::loop::layout_of(argv[1]);
