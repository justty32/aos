...
void merge_environment(const Spawn &spawn, SpawnPrep &prep,
                       const char *&path_value, bool &path_is_set) {
    for (const auto &entry : spawn.env) {
    ...
    for (char **item = environ; item != nullptr && *item != nullptr; ++item) {
        ...
        const auto replacement = spawn.env.find(key);
        if (replacement == spawn.env.end()) {
            prep.environment.push_back(inherited);
        } else if (!emitted[key]) {
...
std::string child_directory(const Spawn &spawn) {
    const std::string parent = current_directory();
    if (parent.empty() || spawn.cwd.empty()) {
        return parent;
    }
    if (spawn.cwd.front() == '/') {
        return spawn.cwd;
    }
    return join_path(parent, spawn.cwd);
}

void resolve_executable(const Spawn &spawn, const std::string &path,
                        SpawnPrep &prep) {
    const std::string base = child_directory(spawn);
    if (base.empty()) {
    ...
    for (;;) {
        ...
        const std::string candidate = join_path(directory, spawn.argv[0]);
...
void prepare_spawn(const Spawn &spawn, SpawnPrep &prep) {
    ...
    merge_environment(spawn, prep, path_value, path_is_set);
    if (prep.failure_status != 0) {
        return;
    }
    if (spawn.argv.empty() || spawn.argv[0].empty()) {
        prep.failure_status = kExitExecFailed;
        return;
    }
    if (spawn.argv[0].find('/') != std::string::npos) {
        prep.executable = spawn.argv[0];
        return;
    }

    const std::string path = path_is_set ? path_value : default_path();
    resolve_executable(spawn, path, prep);
}
