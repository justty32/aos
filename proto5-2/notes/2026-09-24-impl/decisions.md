...
- **D-5 boot 等舊 kernel 池多等 draining 0**（astra P2）：現象——kernel 池縮 0 後真 daemon 會發布 `count 0、running 0、draining 1`，boot 只看 running／killing 就寫新鏈，舊 tick 還活著。改法——boot 等到 summary 確定不在（gone），或 `count 0、running 0、killing 0、draining 0`（摘要讀不到＝還在，見 D-49b）。實作比 handoff §1.2 字面多等 draining 0；spec 原句漏列（同段要「沒有任何一格在跑」），proto5-2 規範不動，翻案要改 `aos_kernel_boot._gone_or_idle`。
...
**kernel 修正隊（astra 唯讀審查 [review-astra.md](review-astra.md) 的必修；P2 見 D-5；kernel 段號碼只剩三個，後兩條用 D-49a／D-49b）**：

- **D-47 astra P1 boot 交接完重讀帳本**：現象——boot 在送縮池單前讀帳本，等舊 kernel 退出期間舊 tick 還能提交（新登記的行程、結清並 ack 的工作），boot 卻拿舊快照寫新鏈，新行程消失、已結清的工作被恢復成 busy 而重跑。改法——交接前的快照只拿來找舊 kernel 池；確認舊主人退出（D-5）後重新讀帳本，在新讀的帳本上改鏈、丟 scale sends、設池旗標。測試：`test_kernel_fix_astra.BootReread`。翻案：`aos_kernel_boot.boot` 第 3 步改回用 `old`。
- **D-48 astra P3 boot 的整份重送與一般 redeclare 分開**：現象——boot 設 `redeclare`，但第一格收到舊鏈的 scale 回音時，成功回音把它清掉（集合相同就不送新宣告）、錯誤回音（TooMany／NameTaken）也清掉並記成 `error`、不再自動重試。改法——帳本池格多 `boot_redeclare`（boot 設 true、`new_pool` 為 false）；它為 true 時，名字不屬於現在這條鏈（`k-<chain>-` 開頭）的回音照收、ack、成功就更新 `sent`，但 `redeclare` 維持 true、不記 `error`；只有這條鏈送出那張的**成功**回音才清掉它（新鏈那張回錯誤時照一般規則記 `error`，旗標留著但不再影響送單）。保守：boot 的重送一定真的送到 daemon 一次。測試：`BootRedeclare` 兩條。翻案：`aos_kernel_pools.collect_scale` 的 `old_chain` 分支、`boot` 的 `entry.update`。
- **D-49 astra P4 從沒取得過的位置直接放棄**：現象——初次宣告撞到別人的池（`NameTaken`，`sent` 空）後人改 `dpool`，kernel 把它當搬池：向別人的池送縮 0、等那池 summary 消失——永遠不會，所以永遠不宣告新位置。改法——帳本池格多 `acquired`（任何成功回音設 true；`new_pool` 為 false；舊帳本沒這鍵當 true，走原本的搬池）。要換位置／移除時，若 `acquired` false、`sent` 空、沒 pending、那池沒 busy，就記 `pool_abandon`、直接換成新位置的新格（或忘掉），本格就宣告新位置，不送縮 0、不看舊 summary。保守：只要 daemon 確認過一次就照原規矩縮 0、等 summary 消失。測試：`NameTakenMove`。翻案：`PoolsMixin.abandon`。
- **D-49a astra P6 交接的「summary 消失」只認確定不在**：現象——`pool_summary` 把讀不到／壞 JSON／不是物件都回 `None`，搬池 retire、boot、halt 都當成池已拿掉而放行。改法——這三處改用 daemon 隊的 `aos_daemon.pool_summary_state`：只有 `gone` 才算消失，`unknown` 一律當還在（搬池記 `pool_summary_unknown` 事件；boot 的 `AlreadyRunning` 與 halt 的 `Timeout` 訊息印出是哪池讀不到或哪格不是 0）；halt 判「kernel 沒在跑」時 kernel 池摘要讀不到也當在跑（照放 stop、照等）。ls／health 照舊用寬鬆的 `pool_summary`。測試：`UnknownSummary` 三條。假 daemon 多 `garble`（縮 0 時把 summary 寫壞），`linger` 的摘要改成真 daemon 的形狀 `count 0、running 0、draining 1`。`test_p52_e2e` 的交接測試原本比 kernel 那顆的 `gen` 變大，現在 boot 多半等到池整個拿掉再重建、`gen` 從 1 重算，改比 pid 不同。翻案：`aos_kernel_pools.pool_summary_state` 改回包 `pool_summary`。
- **D-49b astra P7 通知的 home 解析出錯也是壞通知**：現象——合法 JSON 的 `responded` 通知，`params.home` 含 `\u0000`（`realpath` 拋 `ValueError`）或尾端是 5000 位數字（`int()` 超過轉換上限），整格 tick 丟例外、通知沒刪，之後每格都撞。改法——`_notification` 把 `_notified_cpu` 的 `ValueError`／`OSError` 轉成 `bad_notify`（進 deletes、log 一行），這格不退 1；`split_key` 的號碼字串超過 18 位數字直接不合（回 None）。測試：`BadNotifyParse`。翻案：`aos_kernel_engine.Kernel._notification`、`aos_kernel_info.split_key`。
...
- **D-66 交接用的摘要讀法（review P6）**：撞到——`pool_summary` 把「檔不在」跟「讀不到／壞了」都回 `None`，交接會把讀壞當成池已拿掉。選——新增 `aos_daemon.pool_summary_state(D, dpool)` 回 `("gone", None)`／`("ok", dict)`／`("unknown", None)`：只有 `FileNotFoundError`／`NotADirectoryError`（含池資料夾不在）算 gone；檔在但讀不到、壞 JSON、不是物件、其他 `OSError` 一律 unknown。池名本身不合法＝gone（daemon 從不建這種池，回 unknown 只會讓呼叫端永遠等）。`pool_summary` 行為不變（給顯示）。保守：交接只在確定不存在時放行。翻案：`pool_summary_state` 開頭的名字判斷。
- **D-67 摘要寫／刪池失敗的重試（review P5）**：`write_summary` 寫成功才清 `changed`，失敗留在待發布集合、下一圈再寫；刪池照 summary → pool.json → 資料夾的順序、前一步沒成功不做下一步，沒刪乾淨就記進 `Daemon.removing`（池已從記憶體拿掉，`ls` 看不到、scale 當新池），每圈重試到資料夾消失。錯誤訊息裡有隨機暫存檔名，所以「只印一次」按連續失敗算：一段失敗只印第一次，成功後再壞才再印。待刪中又來同名池的 count>0 宣告＝取消待刪（不然重試會刪掉新池的檔）；count 0 的照舊回 ver 0、不動待刪。停機時待刪沒收完就算了，下次開機 pool.json（count 0）或殘骸資料夾（D-63）會再收。翻案：`aos_daemon_loop.Daemon.publish／new_pool`、`aos_daemon_pools.remove_pool／Pool.write_summary`。
- **D-68 每圈只看有事的池（review P9）**：`Pool.dirty`／`changed` 改成一設 True 就把池登記進 daemon 的兩個有序集合（dict 當有序集合，保留舊的池順序），每圈只處理登記過的；輪流佇列改 `collections.deque`。「count 0 收完」一定伴隨 `changed`（drop／scale／新建都會設），所以刪池判斷也只看待發布集合。閒著時一圈的成本跟池數無關（測試：2000 池、閒著一圈不碰任何池）。崩潰窗口（review P10 daemon 四條：回音後刪原單前、刪 summary 後刪 pool.json 前、killing 號加回寫完 pool.json 後、fork 後寫 kids 檔前）都補了定點測試，現有程式全部照規範收斂，沒找到新 bug。翻案：`Pool.dirty／changed` 改回普通屬性、`step`／`publish` 改回掃 `self.pools`。
- **D-69 刪池後忘掉 decl（review P8，隊長裁定不做 tombstone）**：刪池後忘掉 decl：舊鏈晚到的 scale 會把池建回來。實務上靠 daemon 照檔名順序處理（舊 chain 檔名排前）與 boot 的 kernel 池縮 0 擋掉大半；剩下的窗口（舊鏈殘格的 tick 在 boot 後才放單）列為保證外，翻案要加 tombstone（另存每池最後的宣告序號與 owner，刪池後照樣擋 Stale，並定義 owner 換手何時清掉）。見下面「規範有洞」第 1 條。

**規範有洞、daemon 端處理不了的（kernel 隊要知道）**：
1. （保證範圍見 D-69）池縮到 0 收完就從 daemon 消失，連 `decl` 一起忘掉；之後才被處理的舊單（比現在的 decl 舊）會把池重建——Stale 只擋得住「池還在」的情況。同一圈裡先後處理的兩張不受影響（池在那圈結束才拿掉）。kernel 放單的檔名 `k-<chain>-<seq>-…` 的 seq 沒補零，照字典序處理時 `-10-` 會排在 `-9-` 前面，靠 decl 擋。
...
5. 交接要判「池已完全拿掉」請用 `pool_summary_state`（D-66），不要用 `pool_summary` 的 `None`。
...
### 真跑打撈隊（真跑紀錄字眼修正，D-120～）

打撈 [run.md「碰到的問題」](run.md) 1～6 條：都不是行為 bug，只改 `ls`／`check`／`init` 印出來的字，不改帳本、不碰 daemon 檔。只准改 `aos_kernel_cpu.py`／`aos_kernel_check.py`／`aos_agent_init.py` 三個檔＋對應測試。

- **D-120（碰到的問題 1／4）scale 單「在路上」拆成三種情形**：撞到——同一句「scale 單在路上（等回音）」蓋掉三種完全不同的狀況：①daemon 活著、kernel 在跑，只是帳本要等下一格才收回音（正常，會自己好）；②daemon 沒在跑，單留在路上等它上線；③kernel 已經 `halt`（`phase` 是 `stopped`），這張回音規範上本來就留給下次 `boot` 讀（kernel-pools §…），不是卡住。選——`row_line` 讀 `state.get("phase")`（新塞進 `pool_row` 回的 row，不進 `--json`，因為 `--json` 沒有印這串 tail 訊息，只有 row 本身的欄位）分流：`phase=="stopped"` 印「停機中，下次 boot 收回音」；`daemon_alive` 假印「宣告已送出，daemon 沒在跑（daemon 一上線就會收到）」；其餘印「宣告已送出，下一格確認」。保守：只換 tail 的白話字，不改 `pending`／`daemon_alive`／`phase` 這些欄位本身的判斷邏輯，`--json` 的 row 欄位一個都沒多沒少。翻案要改：`aos_kernel_cpu.row_line`（拿掉 `phase` 分支、改回原句）。測試：`test/test_kernel_cpu.py` 的 `test_pending_scale_order_wording`、`test_error_pending_dead_daemon_moving_removed`（daemon 沒在跑那句同步改了）。

- **D-121（碰到的問題 1）單顆「待宣告」只換印出來的字，不換 `--json` 的 `status`**：撞到——kernel-cli.md §「cpu ls」寫「`--json` 同樣內容」，照字面 `--pool` 每顆的 `status` 應該跟文字行一致；但隊長任務書明講這五處「只改印出來的字，不改行為與 `--json` 欄位」。「待宣告」／「收掉中」本來就不是 protocol／kernel-cli 規範裡的固定詞（規範只點名 `idle｜busy｜draining` 三種），是 kernel 隊自己的決定（D-41），所以改個字不算破協定，但為了不動到任何可能拿 `--json` 的 `status` 字串做比對的呼叫端（daemon 隊、agent 隊、整合測試隊都在同一個 worktree 平行改），**選最保守**：只在文字排版函式 `cpu_line()` 裡做顯示映射（`_STATUS_LABEL = {"待宣告": "等 daemon 確認"}`），`cpu_rows()` 回的 `item["status"]`（`--json` 直接讀這個）一字不動，仍是「待宣告」。代價：文字行跟 `--json` 這一格不再逐字相同（規範原句字面上的期待），只有語意相同。翻案要改：`aos_kernel_cpu.cpu_line` 拿掉 `_STATUS_LABEL`，或改成連 `cpu_rows()` 一起換字（後者才會讓 `--json` 也變成「等 daemon 確認」）。測試：`test/test_kernel_cpu.py` 的 `CpuLineWording` 類、`test_awaiting_cpu_status_wording_text_only_json_unchanged`（同時鎖住文字行換了、`--json` 沒換）。

- **D-122（碰到的問題 3）池行 `draining` 改成即時算，不讀帳本的舊計數**：撞到——`cpu rm` 剛下的那一兩秒，池行讀 `entry.get("draining", 0)`（帳本自己維護的計數，要等下一格 `KernelLedger` 重算才更新），跟同一時刻單顆行（`cpu_rows()`，即時比對 `busy` 是否還在 `members(count, skip)` 裡）算出來的「收掉中」對不上：池行寫 `draining 0`，單顆已經是 `draining`。選——`pool_row()` 裡本來就有算「不在 W 且在 busy」的 `waiting` 清單（給「收掉中 N 顆，等 X」那句 tail 用），池行的 `draining` 欄位直接改用 `len(waiting)`，不再讀 `entry.get("draining", 0)`。這欄位剛好也符合 kernel-cli.md 對 `draining` 的定義（「已經不是成員、還在做事的號數」），所以不是加新語意，是把顯示值換成跟定義更貼的即時算法。**不改帳本**：`KernelLedger` 那份 `entry["draining"]` 計數本身完全沒動，只是這個顯示欄位不再讀它。代價：這一格的 `draining` 會影響 `--json` 的同名欄位值（跟 D-120／D-121 不同，這條**有**動到 `--json` 的值），但任務書第 2 點明講「若只是顯示時機，池行改成即時算出來的數」，判定為授權範圍內的值修正，不是схема變動。翻案要改：`aos_kernel_cpu.pool_row` 把 `draining=len(waiting)` 改回 `draining=entry.get("draining", 0)`。測試：`test/test_kernel_cpu.py` 的 `test_draining_shows_waiting_proc`（新增 `cpu rm` 後、tick 前就檢查池行 `draining 1`）。

- **D-123（碰到的問題 3）收掉那一刻單顆的「daemon pending」改成「daemon 在收」**：撞到——`cpu_line()` 裡 kid 檔讀不到（`aos_daemon.pool_kid` 回 `None`）時一律印「daemon pending」；這個詞在「已宣告、還沒被daemon 拉起來」（例如剛 `cpu add`）跟「已經收掉、daemon 正在清 kids 檔／還沒被讀到」兩種完全相反的情境下都會出現，後者讀起來像還要再拉一顆。選——只在 `item["status"] == "收掉中"` 且 `declared`（`i in s`）時把 daemon 欄位改印「daemon 在收」；其他情況（`busy`／`idle`／`待宣告`）維持「daemon pending」／「daemon -」不動，因為那些情境「pending」原意就對（等 daemon 建檔）。同 D-121，只動 `cpu_line()` 的顯示分支，`item["daemon"]`（`--json` 讀這個，是 `None` 或 `{state,gen,pid}`）不受影響。翻案要改：`aos_kernel_cpu.cpu_line` 拿掉「收掉中」那個 `elif` 分支。測試：`CpuLineWording.test_collecting_with_no_kid_record_reads_as_in_progress_not_pending`、`test_busy_with_no_kid_record_is_unchanged`（鎖住只有「收掉中」變、「busy」不變）。

- **D-124（碰到的問題 5）`check` 的 `daemon` 行分清楚「kernel 設定的池」跟「daemon 目前有」**：撞到——`aos_kernel_check.check()` 印「daemon 活著：D（池 default、llm、kernel）」，括號裡的池名單其實是 `homes[home_d]`（從 K 的 `info.json` 算出「這個 daemon 家被哪些 kernel 池用到」），daemon 剛開、一張 scale 單都還沒收到時也會印同一句，讓人以為 daemon 那邊已經有這些池了。選——daemon 活著且这个 daemon 有對到 kernel 池時（`pools` 非空），逐一用 `aos_daemon.pool_summary(home_d, dpool)`（`dpool` 從 `pool_location(info, pool)` 拿）查那個池的 summary 在不在，組出「daemon 目前有：…」（沒有就寫「還沒有」），跟「kernel 設定的池：…」分兩截印。`--daemon-target` 且池表沒用到（`pools` 空）、daemon 沒在跑兩種情況不變（前者沒有 kernel 池可對、後者本來就不用猜 daemon 有什麼）。保守：只用 team-rules 約定表允許的 `pool_summary`／`is_alive`，不自己掃 `D/pools/` 資料夾、不猜其他沒被 kernel 設定用到的池。翻案要改：`aos_kernel_check.check` 裡 `homes` 那段迴圈，改回原本單一句 `where`。測試：`test/test_kernel_check.py` 的 `test_every_daemon_in_pool_table_is_checked`（改成「還沒有」）、新增 `test_daemon_alive_reports_pools_it_actually_has`。

- **D-125（碰到的問題 6）`aos-agent init` 的提示改指 `proto5-2/README.md`**：`proto5-2/lib/aos_agent_init.py` 印的收尾提示原本寫「見 proto5/README.md 第 2 段」，proto5-2 使用者手上沒有 proto5 的章節編號、對不上。確認 `proto5-2/README.md` 的「十分鐘上手」第 2 段（`## 十分鐘上手` 底下的「**2. kernel：…**」）就是講 `llm.json`／`default` 代號的地方，改成「見 proto5-2/README.md 的「十分鐘上手」第 2 段」。純文字，不影響 exit code／寫檔。測試：`test/test_agent_daily.py` 的 `test_init_valid` 補了兩條斷言（含新路徑、不含舊的 `proto5/README.md`）。

- **D-126（碰到的問題 2，建議、不動 daemon 檔）`aos-daemon ls` 的 `restarting` 容易被誤讀成另一顆**：run.md 記錄「`restarting 1` 加 `running 2` 大於 `want 2`」會讓人以為多了一顆，但 `restarting` 是包含在 `running` 裡的一個子分類、不是互斥分類（daemon-home §… 的定義）。這條**不歸這隊改**——`aos_daemon*.py` 是另一隊的檔，team-rules 明講不准碰；只記錄下來給隊長／daemon 隊參考：`aos-daemon ls` 那行可以考慮改成 `running 2（含 restarting 1）` 之類把包含關係寫進句子裡，不用另外查文件才懂。翻案／要做：改 `aos_daemon_cli.py`（或負責 `ls` 排版的檔）的那一行格式，daemon 隊評估。
