...
5. **讀 `K/requests/`**，列一次目錄，照檔名前綴分：`ack-`（範式 §3.3）、`stop-`（改 `phase`）、`resp-`（回音通知，進第 6 步）、
   其他是 syscall（`add`／`rm`，照 proto5 §2 判）。
6. **收回音**。要查的 cpu＝下面三組的聯集：
   ...
   判完：`busy` 那格拿掉；這號還是成員就放回那池 `free` 的尾巴，已經不是成員（縮小中）就不放回，並標那池「要重算」（[kernel-pools](kernel-pools.md)）。
...
8. **派工**（`phase=running` 才做）：
   1. `delayed` 從頭拿，`not_before` 已到的依序接到各自池的 `ready` 尾巴（排好序，所以一到沒到期就停）。
   2. 每個池：`ready` 不空、`free` 不空 → 兩邊各拿一個配對：行程 `status=running`、`busy[P/i]`＝`{req: k-<chain>-<N>-<P>-<i>.json, proc, discard:false}`、號碼進 `recent`。一直做到其中一邊空。
   3. 這格全部派完**一次寫帳本**，再逐一放檔（先記後放）。崩在中間＝下一格靠 `recent` 補放。
   回 queue（proto5 §4）的行程：`not_before` 已到的接 `ready` 尾巴，沒到的插進 `delayed`（二分插入）。
9. **停機**（`phase=stopping`）：同 proto5 §3 第 9 步的前半——`ready`／`delayed` 裡的 `once` 全部拿掉、各回 `Stopping`；
   `busy` 空、出貨箱空、沒有 `pending`（`procs` 的與池的都算）→ `phase=stopped`，對每個池（含 kernel 池）排一張 `count: 0` 的 scale 單進 `sends`、**寫帳本**。
...
O(syscall 數＋通知數＋`recent`＋`sweep`＋池數＋派工數＋到期數＋出貨數)，**加上寫帳本**（帳本大小仍跟 N 有關，[scale](scale.md) §2）。
沒有任何一步是「每顆 cpu 看一次」。
