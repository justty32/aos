...
`aos-agent exec <世界> [--home DIR]` 一次走一格，七格輪流走：
...
| `wait` | 撿主線 LLM 結果；缺鐘立刻報錯。請求還在 LLM 世界排隊或執行中就一直等（最多 1800 格；等的格不算每題上限），請求不見了結果又沒出現才 60 格放棄。模型把工具呼叫寫成文字：認得的工具救回成正式 `tool_calls`，救不回來就當協定錯誤、同題重送一次 | 撿到 `act`；出錯（缺鐘、等太久、結果帶 error、找不到 choices）一律 `retry`；重送回 `llm` |
...
| `retry` | 對話尾巴那句還沒人回，等著同題重送；照樣掃信箱 | 有新信 `llm`；滿 20 格 `llm`；連錯 5 次 `stuck`；其他留 `retry` |
| `stuck` | 連錯太多次，這句先放著，只等新信 | 有新信 `llm`，沒信留 `stuck` |

**模型出錯不回 `idle`，一律進 `retry`**：那句沒人回的話留在對話尾巴，`retry` 每 20 格同題再送一次；連錯 5 次就轉 `stuck`，喊一句「這句先放著」然後只等新信（新信一來兩格都清帳、重問模型）。等重送、放著等信的那些格都不算每題動作格。最後還有一道**安全網**：任何一格算完要回 `idle`、但對話尾巴還是一句沒人回的話（而且不是睡著、也不是撞上限等人），就自動改成 `retry`——以後誰再多加一條出錯的路，也不會把那句話弄丟。
