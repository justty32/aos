...
- 共用檔的鎖在 `team/.lock`（flock 獨佔，跟 `team/` 其他共用檔共用同一把）。會改東西的工具整支包在鎖裡，`order_status` 只讀不拿鎖；`task_report`／`qa_run` 跑測試那段（最久 60 秒）會先把鎖放掉，跑完再拿回來重讀一次存結果，所以有人在跑測試不會擋住其他人。等鎖最多 10 秒，等不到就回一句「等共用檔的鎖等超過…」。
