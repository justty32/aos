...
5. 做事的人（**chief／dev-a／dev-b**）做完叫 `task_report`：記下改了哪些檔，帶 `test_cmd` 的話
   當場在專案目錄跑一次測試，結果存進任務檔，然後寄摘要給 pm 與 qa。
6. **qa** 想自己再跑一次就叫 `qa_run`，判定就叫 `qa_verdict`。所有任務都 passed 時，單子自動變 `qa`。
...
| `task_report(task_id, summary, files?, test_cmd?)` | chief／dev | 回報做完：記檔案、跑一次測試、寄摘要給 pm 與 qa。 |
| `qa_run(task_id? / order_id?, command)` | qa／pm | 在專案目錄跑一個指令，exit 與輸出尾巴存進任務檔。 |
| `qa_verdict(task_id, passed, evidence?)` | qa | 判這個任務過或不過；全部過了單子就進 `qa`。 |
...
**誰能用哪個**照上表擋：dev 叫 `deliver` 會被回一句「你不能用」。owner 是老闆，什麼都能用（收爛攤子用）。
