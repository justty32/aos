...
PROMPT = ("先 inbox_sources 看哪個來源有未讀，再 inbox_list 看有哪幾封。"
          "讀過的信會搬進 read/，看到了就要處理掉。")

TOOLS = [
    {"name": "inbox_sources",
     "description": "看信箱有哪些來源，各有幾封未讀、幾封讀過。",
     "parameters": {"type": "object", "properties": {
         "unread_only": {"type": "boolean", "description": "只列還有未讀的來源"}}}},
    {"name": "inbox_list",
     "description": "列一個來源的未讀信：id（檔名）、時間、寄件人、開頭一小段。",
     "parameters": {"type": "object", "properties": {
         "source": {"type": "string", "description": "來源名字，例如 user、team"}},
         "required": ["source"]}},
    {"name": "inbox_read",
     "description": "讀某來源的某一封未讀信，讀完搬進 read/。",
     "parameters": {"type": "object", "properties": {
         "source": {"type": "string", "description": "來源名字"},
         "id": {"type": "string", "description": "inbox_list 給的完整檔名（含 .json）"}},
         "required": ["source", "id"]}},
    {"name": "inbox_read_all",
