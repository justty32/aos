...
- **G（自動補 `wf_lint_strict`）代裁**：選在 `handoff` 這一層機械補，不改門房規則——門房、`routes.json` 是 A 隊地盤（T-route），這裡改動範圍最小，且不管請求有沒有經過門房都補得到（領隊自己開單、心跳照例行開單一樣有效）；判法是 goal／facts／workflow 合起來的文字裡同時有 `.md` 與一個改寫用詞（白話、改寫、更好懂、更易讀、講白話），已經放了就不重複加。使用者要翻案（改成門房規則）再議。
