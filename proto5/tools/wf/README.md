...
- **跟事實表的鍵比**，一層一層來：一樣 → 同義詞表（例如「一句話描述」＝「專案一句話」、「測試 / build / lint 指令」＝「驗證指令」）→ 一邊包含另一邊（這層才要至少兩個字）。每一層**恰好一條**才填；某層兩條以上＝不填、也不再拿表格第一格去猜，列「more than one fact looks like it」。
...
- `drop_examples`：講「範例」的〔導入判斷〕，認得出範圍的才刪——上方表格第一格是「（範例）」的列，或底下「下面 N 段」緊接著的 N 個同級小節（沒寫段數、或小節不夠 N 個就不動）——連同那段判斷一起刪。認不出的不動、列出來。
