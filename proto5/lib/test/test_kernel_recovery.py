...
- boot 不再 spawn 每顆 cpu：交接 kernel 池、重宣告工作池（handoff §1）；用 _kernel_fake 的假 daemon 回 scale 音。
...
class RecoveryTests(KernelCase):
    ...
    def test_dispatch_records_before_put_then_next_tick_recovers(self):
        ...
        with patch.object(home, "post_request", side_effect=crash_on_work):
            with self.assertRaises(Crash):
                self.step()
        recorded = home.read_state(self.k)                              # 提交點 3 已寫
    ...
    def test_next_tick_placed_before_last_seq_record_is_idempotent(self):
        name = "k-1000-1-8.json"
        request = {"jsonrpc": "2.0", "id": name[:-5], "method": "aos-exec",
                   "params": {"target": str(CLI), "args": ["tick", "--target", str(self.k), "--chain",
                              "1000-1", "--seq", "8"], "timeout_ms": 0}}
        cpu = self.cpu("kernel/0")
    ...
    def test_tick_request_uses_plain_cli_args_and_no_timeout(self):
        self.step()
        request = home.read_json(self.cpu("kernel/0") / "requests" / "k-1000-1-8.json")
        self.assertEqual(request["params"], {"target": str(CLI), "args": ["tick", "--target", str(self.k),
                          "--chain", "1000-1", "--seq", "8"], "timeout_ms": 0})
    ...
    def test_crash_after_delivery_before_outbox_clear_is_safe(self):
        """第 4 步出貨完、提交點 2 寫帳本前崩：下一格照帳本重放，對方收到的效果一樣。"""
        cpu = self.populate_outboxes()
        ...
        with patch.object(self.engine, "save", side_effect=save):
            with self.assertRaises(Crash):
                self.step()
        self.assertEqual(home.read_state(self.k)["deletes"], ["old-syscall.json"])   # 帳上還在
        self.load_engine(seq=8)
        self.step()
        self.assert_delivered(cpu)
        self.assertEqual(len(list((self.d / "requests").glob("k-*.json"))), 1)
    ...
    def test_stopped_tick_only_flushes_outboxes(self):
        ...
        with patch.object(kernel.Kernel, "pools_step", side_effect=AssertionError("停機後不能動池")):
            self.assertEqual(kernel.tick(self.k, "1000-1", 9), 0)
        self.load_engine()
        self.assert_delivered(cpu)
        self.assertTrue((self.k / "requests" / "later.json").exists())
        self.assertEqual(list((self.cpu("kernel/0") / "requests").glob("k-*.json")), [])
    ...
    def test_first_boot_initializes_missing_ledger_and_writes_raw_envs(self):
        ...
        self.assertEqual(set(state["pools"]), {"kernel", "default", "llm"})
        ...
        self.assertEqual(fake.pool("kernel")["count"], 1)
        self.assertIsNone(fake.pool("default"))                             # 工作池第一格才宣告
        next_path = fresh / "pools" / "kernel" / "cpus" / "0" / "requests" / ("k-%s-1.json" % state["chain"])
    ...
    def test_reloaded_tick_ms_controls_sleep(self):
        ...
        with patch.object(aos_kernel_engine.time, "sleep") as sleep:
    ...
    def test_init_recovers_partial_home_without_overwriting(self):
        ...
        kernel.init(fresh)
        self.assertEqual(kernel.load_info(fresh)["pools"]["kernel"]["count"], 1)
...
class LiveRecoveryTests(LiveKernelCase):
    ...
    def test_boot_preserves_complete_cpu_custom_inst_and_info(self):
        ...
        engine.ensure_cpu_home("default", 0)
        cpu = self.cpu_home("default", 0)
