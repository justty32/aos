...
**互引與名詞。** 確定錯引是 [cpu.md:62](../../spec/cpu/layout.md)：kernel 命名法應指 **§1.3**，不是 §1.2。`killing` 是 daemon 單一孩子狀態，`stopping` 是 daemon 全域旗標或 kernel phase，正文沒有混用；「偷看」也一致是唯讀，但不能因此推導資料仍新鮮。
...
**daemon 自己的家。** [daemon §3](../../spec/daemon/methods.md)套 CPU current／對帳／ack，在檔案協議層成立：已發布回音保留，未發布補 Interrupted。但「副作用先於回音」只描述發生順序，**沒有解決 fork 與持久登記的窗口**；CPU 對帳不會替 daemon 找回未知 PID。

**普通檔 tick。** 依 [aos-exec 三種目標](../../spec/aos-exec/usage.md)及 [CPU fd 搬移](../../spec/cpu/lifecycle.md)：
...
- [kernel boot 第 4 步](../../spec/kernel/cli.md)說每顆 CPU 建家並寫 info／inst，但第 2 步只停 kernel CPU；仍活著的工作 CPU 不能按 [§1](../../spec/kernel/home.md)被重新初始化。應明訂既有家只驗證／補缺，哪些情況才可改寫。
- [aos-exec.md:38](../../spec/aos-exec/usage.md)的「缺檔之後出現就自然跑」與 daemon 首次 SpawnFailed 不登記矛盾；[aos-exec.md:105](../../spec/aos-exec/api.md)也仍把反覆執行交給已退休的 aos-run。

## 4. C 新機制與剩餘漏洞

### K3-1｜syscall 去重憑據會被另一則 syscall 刪掉

**在哪：**[kernel §2](../../spec/kernel/syscall.md)。  
...
**在哪：**[rm 三分支](../../spec/kernel/syscall.md)、[discard 判定](../../spec/kernel/tick.md)。  
...
**在哪：**[出貨](../../spec/kernel/tick.md)、[停機](../../spec/kernel/tick.md)、[boot](../../spec/kernel/daemon-link.md)。  
...
**在哪：**[info 熱改](../../spec/kernel/home.md)、[接鏈與 spawn](../../spec/kernel/syscall.md)。  
...
**在哪：**[kernel boot](../../spec/kernel/daemon-link.md)、[不重疊宣稱](../../spec/kernel/cli.md)。  
...
**在哪：**[boot 第 2 步](../../spec/kernel/daemon-link.md)、[stopped 早退](../../spec/kernel/syscall.md)。  
...
**在哪：**[daemon spawn](../../spec/daemon/spawn.md)、[啟動接手](../../spec/daemon/lifecycle.md)。  
...
**在哪：**[method](../../spec/daemon/spawn.md)、[主迴圈](../../spec/daemon/methods.md)。  
...
**在哪：**[daemon §3](../../spec/daemon/methods.md)。  
...
**在哪：**[daemon 啟動第 3–5 步](../../spec/daemon/lifecycle.md)、[kernel 補拉](../../spec/kernel/tick.md)。  
...
**在哪：**[daemon 階梯](../../spec/daemon/loop.md)、[接手](../../spec/daemon/lifecycle.md)。  
...
**在哪：**[kernel boot](../../spec/kernel/daemon-link.md)、[daemon state](../../spec/daemon/home.md)。  
...
**在哪：**[kernel 第 7 步](../../spec/kernel/tick.md)、[daemon spawn](../../spec/daemon/spawn.md)。  
...
**在哪：**[kernel tick request](../../spec/kernel/tick.md)、[aos-exec 普通檔](../../spec/aos-exec/usage.md)。  
...
**在哪：**[daemon pipe](../../spec/daemon/spawn.md)、[CPU 控制輸入](../../spec/cpu/stop.md)。  
...
| 編號 | 發現與建議 |
|---|---|
| **R3-1** | [CPU 名詞表 base](../../spec/cpu/terms.md)仍一概說「目標所在資料夾」；資料夾目標應是自身。命名導引 §1.2 改 §1.3。 |
| **R3-2** | [request 定義](../../spec/cpu/terms.md)把所有 request 說成執行工作；[daemon 一句話](../../spec/daemon/README.md)又把三種 target 窄化成 inst。CPU 一句話加「工作 request」，daemon 改稱 target；kernel 一句話主旨清楚。 |
| **R3-3** | [req／在途／出貨箱](../../spec/kernel/terms.md)與正文不完全一致：req 非 null 可是已記未放；出貨箱也可能已放未清。應稱「尚未結清」，別一律稱「還沒放出去」。 |
| **R3-4** | [所有檔從帳本出去](../../spec/kernel/README.md)、[所有出貨先記再放](../../spec/kernel/tick.md)都有例外：接鏈先放後記、tick ack 不入箱。第 5／6／9 步新增 replies／acks 若統一留待**下一格第 4 步**，流程可行且停機會多等一格，應直接寫出；不要指向當格已走過的步驟。 |
| **R3-5** | [kernel §10](../../spec/kernel/choices.md)仍把已拍板的 aos 計數政策列待確認；[daemon §9](../../spec/daemon/choices.md)把 NameTaken 與作者選的同步 spawn 混列。另 kernel §4「三個計數」應改兩個。 |
| **R3-6** | [斷鏈診斷](../../spec/kernel/tick.md)需要 kernel CPU 的 requests／current，但 [ls 契約](../../spec/kernel/daemon-link.md)只讀 K、D state。補讀取範圍，或明寫另行人工查檔。 |
| **R3-7** | [daemon 名詞表](../../spec/daemon/terms.md)「訊號退出都重拉」漏 restart／主動停止條件；[先停 daemon 的說明](../../spec/daemon/shutdown.md)及硬砍摘要也不能一概稱 Interrupted。應按 CPU 對帳表分情況。 |
| **R3-8** | [全部檔名帶 chain](../../spec/kernel/ledger.md)有未列例外：boot 使用 stop-boot-epoch；給 daemon 的 ack 命名也未列。應限定宣稱範圍，補全命名規則及 ack 序號 n 的配置範圍。 |
| **R3-9** | [daemon 孩子表「唯一記憶」](../../spec/daemon/terms.md)、[沒有只在記憶體的東西](../../spec/daemon/home.md)說得過滿：restart 到期時間、階梯進度沒有列入 schema。應區分持久身分與可丟棄、重啟時重建的執行狀態。 |