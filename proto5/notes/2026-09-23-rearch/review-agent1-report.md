...
[放單與收回流程](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:71) 沒有明確區分三個名字：
...
[工具結果表](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:97) 把非 `Interrupted` 的 error 都列為「跑不起來」。但 [kernel 的 rm 契約](/home/guanyu/projs/aos/proto5/spec/kernel.md:191) 明定：running once 被 rm，當下就回 `Removed`，底層工作仍可能繼續。
...
[agent 的 `_meta` 規則](/home/guanyu/projs/aos/proto5/spec/agent.md:99) 一概說中心是 agent 家，與 [inst-posix §3.1](/home/guanyu/projs/aos/proto5/spec/inst-posix.md:92) 不同：agent 家是 base；先解 cwd，之後串流與一般 `$ref` 以 cwd 為中心。

> 工具 inst 的 base 是 agent 家；cwd 及其他欄位依 inst-posix §3.1 解析。agent 補入的 stdin／stdout 使用工作區的絕對路徑；argv 的一般參數不擅自當成路徑改寫。

另一個缺口是 [think 的 timeout 來源](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:72)：agent 要讀 llm.json 的 timeout，但 [llm.json 會整份展開](/home/guanyu/projs/aos/proto5/spec/aos-llm-call.md:30)，API key 可能只存在 llm 工作 cpu。若共用完整 loader，agent 送件前就可能因缺金鑰失敗。須定義 agent 可取得 timeout 的來源／解析方式，不能假設兩個池環境相同。

**X-5｜「孤兒回音到 boot 才清」沒有下層依據。〔要修〕**

[aos-agent 自癒段](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:109) 的敘述錯誤。[kernel boot](/home/guanyu/projs/aos/proto5/spec/kernel.md:306) 保留在途帳本與 pending，沒有清 `K/responses/` 的步驟。
...
[工具結果表](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:97) 要輸出原因，但 [exec result](/home/guanyu/projs/aos/proto5/spec/cpu.md:182) 只有 `code/kind/timed_out/stopped/ms`；只有 RPC error 才有 `message`。
...
[工作名](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:21) 直接以資料夾名開頭；家叫 `ack-bob`／`stop-bob` 時，会撞 [管理 request 前綴](/home/guanyu/projs/aos/proto5/spec/cpu.md:207)。加固定安全前綴即可，例如 `work-<agent>-<ns>-<pid>`。

[「程式寫檔一律 tmp 再 rename」](/home/guanyu/projs/aos/proto5/spec/agent.md:42) 也過度概括：
...
[託管範例](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:129) 少了位置參數：
...
- **時序：** 回音到 → [§2 劃掉 waits 並寫回](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:35) → §3 又只靠 waits 判在途。
...
- **時序：** [記憶 → ack → 清檔 → state](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:78)，任一縫崩潰。
...
- **時序：** [寫 inst／arguments → 部分 add 已送 → waits 未寫就崩](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:85) → 下次換 N 整批再送。
...
- **時序：** [工具不存在／`_meta` 解失敗／本地送件失敗](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:84)，其他 call 成功送出 → waits 只存已送路徑 → 退出。
...
- **時序：** 工作 running → rm → `Removed` 到 → [agent ack 並刪 inst／in／out](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:101) → 工作仍讀寫。
...
- **時序：** [continue.json rename 成 .done](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:35) → waits 尚未寫回就崩。
...
- **時序：** [失敗 → errors 加一 → ack → state 未寫就崩](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:52)；或 exec 成功但 message 無效，退 1、不 ack。
...
- **時序：** [接記憶 → rename 輸入 → state 改 think](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:47)。
...
- **時序：** 已送 once → [kernel stopping](/home/guanyu/projs/aos/proto5/spec/kernel.md:238) → queued 回 `Stopping`，running 收完；agent 的反覆行程不再派送。
...
- **時序：** [同家用兩個行程名登記，或排程中手動再跑](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:129) → 各讀舊 state／history → 交錯覆寫。
...
舊版送出 agent 已組好的 body；新版[只傳 agent 路徑](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:65)，等 llm 工作執行時才讀檔。
...
[目前名詞表](/home/guanyu/projs/aos/proto5/spec/aos-agent.md:12) 建議補：
...
[欄位表](/home/guanyu/projs/aos/proto5/spec/agent.md:115) 允許單條，[§4.2](/home/guanyu/projs/aos/proto5/spec/agent.md:134) 卻要求原始值必為陣列。
...
[llm 設定](/home/guanyu/projs/aos/proto5/spec/aos-llm-call.md:25) 有 `_metainfo` 範例，卻沒寫是否必填、如何驗；`api_key` 沒完整型別；正整數也應明說 bool 不算。`ConfigInvalid` 並非舊 llm-cpu 設定錯誤的原名，舊文用的是 `EngineInvalid`。

建議至少補：

> api_key 可省、null 或字串；timeout_ms 為正整數，bool 不算。明訂 llm_config 的身分與版本驗證，以及設定讀取、JSON 語法、指示詞錯誤各用何種代號。

[模型輸出](/home/guanyu/projs/aos/proto5/spec/aos-llm-call.md:51) 也不能只檢查 message 存在：
...
[人格／記憶段](/home/guanyu/projs/aos/proto5/spec/agent.md:82) 把兩個小節合成標題，卻被別處當獨立 §3.2 引用；舊版又預計被取代。應拆回獨立小節，直接保留必要驗證條款，去掉「一字不改」等版本依賴。
