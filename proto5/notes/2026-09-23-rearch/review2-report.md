...
- `kind=aos, code=1` 是 API 結果，CLI 才換成 125，沒有打架。依據：[cpu §4.1](../../spec/cpu/methods.md)、[aos-exec 三種目標與旗標](../../spec/aos-exec/usage.md)、[退出碼](../../spec/aos-exec/exit.md)。

**C2-1｜ack 本身的消耗與恢復順序沒有寫完**

**在哪：**[cpu §3.3](../../spec/cpu/messages.md)、[§6.3](../../spec/cpu/lifecycle.md)。  
**時序：**
...
**在哪：**[cpu 身分與放單](../../spec/cpu/messages.md)、[ack](../../spec/cpu/messages.md)。  
...
**在哪：**[cpu §6.2](../../spec/cpu/lifecycle.md)。  
...
**在哪：**[cpu §5.1–5.3](../../spec/cpu/stop.md)、[§6.3](../../spec/cpu/lifecycle.md)。  
...
**在哪：**[cpu §6.3–§7](../../spec/cpu/lifecycle.md)、[kernel daemon 契約](../../spec/kernel/tick.md)。  
...
**在哪：**[cpu §3 信封與錯誤表](../../spec/cpu/messages.md)、[§4.3](../../spec/cpu/methods.md)。  
...
**在哪：**[kernel §3 第 2 步](../../spec/kernel/tick.md)、[第 9 步](../../spec/kernel/ledger.md)、[boot](../../spec/kernel/tick.md)。  
...
**在哪：**[kernel §3 第 2 步](../../spec/kernel/tick.md)、[CPU 刪單順序](../../spec/cpu/lifecycle.md)。  
...
**在哪：**[kernel boot 第 1–3 步](../../spec/kernel/tick.md)、[不重疊宣稱](../../spec/kernel/tick.md)。  
...
**在哪：**[kernel req 命名](../../spec/kernel/ledger.md)、[boot 保留狀態並重設 next](../../spec/kernel/tick.md)。  
...
**在哪：**[kernel §3 第 6 步](../../spec/kernel/tick.md)、[cpu 發布與對帳](../../spec/cpu/lifecycle.md)。  
...
**在哪：**[kernel syscall 與重掃規則](../../spec/kernel/home.md)。  
**時序：**
...
**在哪：**[kernel 第 6 步](../../spec/kernel/ledger.md)、[§4 判定](../../spec/kernel/echo.md)。  
...
**在哪：**[kernel rm](../../spec/kernel/home.md)、[第 5–8 步](../../spec/kernel/ledger.md)。  
...
**在哪：**[kernel pending 接受](../../spec/kernel/ledger.md)、[第 8–9 步](../../spec/kernel/ledger.md)。  
...
**在哪：**[kernel state 範例](../../spec/kernel/home.md)、[第 4–6 步](../../spec/kernel/ledger.md)、[停機條件](../../spec/kernel/ledger.md)。  
...
**在哪：**[kernel §4](../../spec/kernel/echo.md)。  
...
**在哪：**[kernel info.cpus](../../spec/kernel/terms.md)、[add](../../spec/kernel/home.md)、[tick](../../spec/kernel/ledger.md)、[boot](../../spec/kernel/tick.md)。  
...
**在哪：**[kernel 第 9 步](../../spec/kernel/ledger.md)、[restart 契約](../../spec/kernel/tick.md)、[CPU 正常停退 0](../../spec/cpu/stop.md)。  
**時序：**
...
**在哪：**[kernel 行程紀錄](../../spec/kernel/home.md)、[派工](../../spec/kernel/ledger.md)、[cpu args 型別](../../spec/cpu/methods.md)。  
...
**在哪：**[cpu 缺檔分類](../../spec/cpu/methods.md)、[kernel aos 退件](../../spec/kernel/syscall.md)、[aos-exec 缺檔用途說明](../../spec/aos-exec/usage.md)。  
...
**在哪：**[kernel spawn 契約](../../spec/kernel/tick.md)、[預設 daemon](../../spec/kernel/tick.md)。  
...
**在哪：**[cpu result](../../spec/cpu/methods.md)、[aos-exec API](../../spec/aos-exec/api.md)。  
...
**在哪：**[cpu §6.1](../../spec/cpu/lifecycle.md)。  
...
[cpu 名詞表](../../spec/cpu/terms.md)說 request 指向 inst；[kernel CLI](../../spec/kernel/tick.md)又把 base 概括為 TARGET 所在資料夾。應統一稱 aos-exec target，並直接連到三種目標表；資料夾目標的 base 是資料夾本身。

**R2-2｜五列表可讀，但把推論寫成唯一歷史。**  
[cpu §6.2](../../spec/cpu/lifecycle.md)的「上一任死在」宜改為「可能停在哪裡」。表應突出觀測條件與必做動作，另列 notify、名稱不重用等前提，避免「不可能」「各對應一段」掩蓋 C2-3。

**R2-3｜十步標籤有幫助，最重要的寫入卻仍藏在動詞裡。**  
[kernel §2](../../spec/kernel/syscall.md)的「做副作用」及[§3 第 6 步](../../spec/kernel/tick.md)的「判定」包含寫回音、刪 procs、搬 done／bad。應在各自章節展開提交點與重入子步驟，再由十步引用；「每步都寫了下格怎麼接」目前不實。

**R2-4｜慣例、規定、已拍板仍有三種聲音。**  
[cpu §3.3](../../spec/cpu/messages.md)稱 ack 前綴只是慣例、認 method；[§10](../../spec/cpu/choices.md)又稱前綴是規定。後者還把已拍板的 target 欄位列待確認。依本次前提，正文應統一為前綴規定，並移除重複待確認項。

**R2-5｜kernel 繼承哪些 cpu 方法，沒有明確入口。**  
[kernel syscall 表](../../spec/kernel/home.md)沒有 ack；tick 只明寫向 CPU 發 ack，沒有明列處理 `K/requests/ack-*` 的位置。應直接說明繼承 cpu §3.3，並在步驟 5 指出；kernel CPU 自己的回音則獨立導引，接上 K2-10。

**R2-6｜FIFO 與斷鏈診斷的用語仍會誤導。**  
[kernel §7](../../spec/kernel/no-overlap.md)稱 FIFO，但 CPU 實際按檔名字典序；只有單一後繼時看起來等價。[第 2 步](../../spec/kernel/ledger.md)說斷鏈「ls 看得出」，但 CLI ls 本身要活 tick 處理，鏈斷時只會逾時；回傳格式也沒有即時 current／目錄空狀態。應明指讀哪份狀態或哪個目錄。
...
| **3. 同名 spawn 的等價條件是什麼？target／restart 不同仍可回原 pid 嗎？** | kernel 依賴同名重送不會产生第二個主人；逾時不代表未做。[kernel §5](../../spec/kernel/daemon-link.md) |
...
| **7. restart 如何節流？持續無法啟動或寫回音時，如何呈現及何時再嘗試？** | CPU 可因永久 I/O 錯誤在啟動對帳時反覆退 1。[cpu §6.3–§7](../../spec/cpu/lifecycle.md) |
...
| **10. 每條控制 pipe 的各端點由誰持有、何時關閉，如何保證某孩子的寫端不漏給其他孩子？** | CPU 把所有寫端關閉視為 stop；漏端會破壞 EOF。[cpu §5.1](../../spec/cpu/stop.md) |
...
| **17. kernel stop 與 daemon stop 的順序由誰保證？daemon 先停時，誰收完 once？** | kernel stop 不停 daemon；已拍板要求在途與 once 收完才停 CPU。[kernel §2、§3](../../spec/kernel/syscall.md) |
