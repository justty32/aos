...
← [rearch README](README.md)｜規範：[agent.md](../../spec/agent/README.md)、[aos-agent.md](../../spec/aos-agent/README.md)、[aos-llm-call.md](../../spec/aos-llm/README.md)
...
13. **C-10（同一個家兩個驅動者）沒寫測試。** aos-agent.md §13 列為保證外、沒有鎖；C-1～C-9 都有測（`test_agent_crash.py`，C-9 另有真 kernel stop／boot 版在 `test_agent_integration.py`）。
14. **真跑時模型多回的 `reasoning_content` 進了記憶。** LM Studio 的 gemma-4 每則 assistant 都帶 `"reasoning_content": ""`；照 agent.md §3.2「其他 key 原樣留著」存進記憶、下一問原樣送回，LM Studio 照收。
   這次是空字串無害；推理模型若回長推理，記憶會一起長大並回送（見下「要使用者拍的」）。
15. **`wf/session_logs/2026-09.md` 留 3 條指向已刪檔的連結。** 那是 hub 檔、這次不碰；刪檔後 wf-lint 總數仍降（916→910）。

## 要使用者拍的

1. **模型回的非標準欄位（例如 `reasoning_content`）要不要在收回時拿掉？** 現行照規範原樣存、原樣回送；拿掉會讓記憶跟模型原話不一致，不拿則推理內容會累積並回送。
2. **封存檔 `*.<消費 id>.done` 誰清、何時清？**（第 3 輪已提過，仍開著）真跑每收一次輸入就多一個，agent 不清。
