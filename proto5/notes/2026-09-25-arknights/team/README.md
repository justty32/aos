...
單人（老何塞）**第 2 次交件過**（第 1 次是我設定錯、不是寫手錯）；一批 3 人（老木頭、老薑、老財）****沒跑完**（品管補：老薑 done；老木頭交件、驗收 12 條過，但審查單卡在 working；老財沒寫出檔案；11:34 起整隊沒再叫模型，`run.py` 也沒存紀錄，見 [qa/](../qa/README.md)）**。
...
| batch r1 `20260925-112507-batch` | 補一批 老木頭、老薑、老財 | **沒跑完**（品管補，12:01 看的狀態；紀錄夾只有 `hops.jsonl`） | 老薑：寫手 2 次、審查 2 次（第 1 次擋 C 引用漏行）後 done；老木頭：寫手 1 次、審查沒審完；老財：0 | 97（14／26＋39／18） | 3,545,139（領隊 93,909、writer-1 929,403、writer-2 1,350,465、審查 1,171,362） | 老薑開單→done 7 分 52 秒；之後 11:34 起停擺 | 0 |
...
- 品管補（batch r1，`aos_hops.py report`）：writer-2（老薑）等模型 61 次共 270 秒；writer-1（老木頭＋老財）47 次共 171 秒；審查員 18 次共 171 秒；每個成員都有 4～5 次「退 102 → 300 秒後再派」＝停車。11:34 之後沒有任何模型呼叫，停擺原因品管沒查。
...
| batch r1 老薑（品管補） | 12 條機械全過 | 第 1 次擋 #9：「證據第 13、19、22 行的 C 引用漏行：百灶包裹在原文第 76 行，代理老闆等在第 77 行」；#10、#11 過。第 2 次 3 條全過。 |
| batch r1 老木頭（品管補） | 12 條機械全過 | 審查單 t-0001.r1 停在 working，沒有判語 |
| batch r1 老財（品管補） | 品管補：無資料（沒交件） | 品管補：無資料 |
...
## 8. 六軸打分（one r1；batch r1 沒跑完，品管補：不打分）
...
6. 評分隊的 `eval.sh` 只認 golden 裡的批（都是已 commit 的人），而我照指示從基準集以外挑人，所以**沒跑 eval.sh 的評審那塊**；品管補：品管部已用包裝腳本（評審參照改用草稿）評了老薑、老木頭，見 [qa/](../qa/README.md)。

**留下一輪**：§7「沒修」5 條；評審（opus）對這批產出的分數；品管補：batch r1 在 11:34 起停擺的原因、`run.py` 行程沒走到存檔就不在的原因，都還沒查。
