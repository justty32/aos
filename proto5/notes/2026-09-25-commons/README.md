...
- astra 必修 1（抄附件的時間差）、5（入庫崩在一半的恢復），見 §7.1。
...
- 新測試 `lib/test/test_team_commons.py` 38 條（下面 29 條＋審查修正 9 條）：掛載三層（預設開、團隊關、成員蓋過、重跑 init 拿掉、自訂位置與壞值、commons 在專案裡被擋）、投稿端擋（缺欄位、絕對路徑、`..`、不在、符號連結、太大、經驗附檔）、圖書館端機械擋（缺欄位、壞路徑 4 種、太大、執行位、符號連結、完全重複）、入庫後 index 與 INDEX.md、像的叫一次、真跑那一對、非圖書館員寄判決被郵差退、沒叫判的不收、兩隊＋圖書館員隊經真郵差走一圈、工具與 lib 查法一致、**成員在真 bwrap 牢裡寫 commons 被擋**（對照：自己 outbox 寫得進）、CLI、匯入 playbook 兩次。
- 全套：修審查前 2694 條全綠（257 秒）；修完再跑一次，結果見 §7.1。

### 7.1 astra 唯讀審查（[review-astra.md](review-astra.md)，必修 11 條，修 9 條）

| # | 問題 | 處理 |
|---|---|---|
| 1 | 郵差抄附件：查完路徑到照路徑抄之間，成員換掉檔或連結，可能抄到牢外的檔 | **留下一輪**（要改成描述符逐層開、不跟連結）；規格 §8 明列 |
| 2 | 專案放在 commons 裡面、或可寫的多掛指到 commons，工人就能改館藏 | 修：init 雙向擋 `BadCommons`；可寫多掛碰到 commons＝`AccessUnsafe` |
| 3 | 別隊或同隊沒被指定的圖書館員也能判 | 修：`judge.json` 的隊與成員要對上，不然 `NotAsked` |
| 4 | 回信先記「已寄」才開 notice，崩在中間就永遠漏寄；一筆出錯會吞掉同輪其他筆 | 修：notice 開好才記已寄；圖書館與回信兩半、每一筆各自出錯各自略過 |
| 5 | 入庫崩在「index 寫了、結果沒寫」：重試會誤判重複或多入一份 | **留下一輪**；規格 §8 明列 |
| 6 | 兩份一樣的同時等判，兩份都能被收 | 修：accept 前在鎖裡再比一次完全重複 |
| 7 | 附件 `README.md/x` 讓入庫每輪失敗、擋住後面的投稿 | 修：第一段是 `README.md` 就 `BadPath`；desk 一份出錯不擋後面 |
| 8 | 投稿發布後、紀錄前崩潰，重試又去讀原附件 | 修：已發布就照已發布的補紀錄（投稿號被別份用了＝`AlreadyExists`） |
| 9 | `Team-A`／`team-a`、不同上層同名隊的代號會撞 | 修：代號＝名字＋實際路徑雜湊 6 碼 |
| 10 | `import` 撞到同名的人工條目會刪掉它 | 修：只換「來自 playbook」的條目，其他撞名＝`Conflict`；也比標題、標籤、適合 |
| 11 | 多隊同時第一次建 index，後建的空索引可能蓋掉已入庫的 | 修：用「只建不蓋」（link）建 index |

建議採了兩條（規格補上「像」的新判法；import 也比標題、標籤、適合）；「小集合只碰一個泛用詞就叫模型」與「自訂模板沒 task 包時工具開關不生效」留下一輪。每條修正各有一條測試（`ReviewFixTests`，9 條），commons 測試共 38 條。
