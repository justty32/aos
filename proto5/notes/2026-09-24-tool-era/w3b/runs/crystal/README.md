...
領隊處理這 20 句：想了 59 次、524k token（prompt 518k＋completion 7k）、模型時間 63 秒，約每句 3 次、26k token。

| 版本 | 候選 | 正例（12） | 負例誤觸（8） | 刁鑽句誤吃（8） | token | 秒 |
|---|---|---|---|---|---|---|
| 沒候選（範例規則） | 0 | 0/12 | 0/8 | — | 0 | — |
| 機械 | 4 | 12/12 | 0/8 | 2/8（`../` 開頭的路徑 2 句；工人在牢裡，出不了專案） | 0 | 0.04 |
...
五隊合起來：落穿 5（沒命中 3、否定 2），5 句都靠原文＋時間對回了信；教程那句「在專案建一個 hello.md，第一行寫「# 你好」…」
