...
1. **[P1] 投遞契約漏掉「已被收走」的去重規則。**  
   位置：[mail.md:34](../../../spec/team/mail.md)、`aos_agent_say.drop_new`。  
...
2. **[P1] `open_review` 不符合「同 src 回同一份動作」。**  
   位置：[aos_team_task.py:341](../../../lib/aos_team_task.py)。  
...
3. **[P1] 逾期處理有漏通知窗口。**  
   位置：[aos_team_task.py:442](../../../lib/aos_team_task.py)。  
...
4. **[P1] 回答問題可以錯誤恢復另一件等待。**  
   位置：[aos_team_ask.py:43](../../../lib/aos_team_ask.py)、同檔 71 行；`aos_team_task.py:202–209`。  
...
5. **[P1] 額外掛點沒有團隊層級的保護，能破壞身分牆。**  
   位置：[aos_agent_init.py:66](../../../lib/aos_agent_init.py)、`aos_agent_access.py:339`。  
...
6. **[P1] init 有兩個無法正確補完的崩潰窗口。**  
   位置：[aos_agent_init.py:158](../../../lib/aos_agent_init.py)、同檔 128 行；`aos_agent_tools.py:201`。  
...
7. **[P2] 投遞／收件事件沒有綁定真正的派工信。**  
   位置：[aos_team_task.py:432](../../../lib/aos_team_task.py)。  
...
8. **[P2] 驗收／審查事件缺版本仍被接受，與規格矛盾。**  
   位置：[aos_team_task.py:146](../../../lib/aos_team_task.py)、同檔 213、227 行。  
...
9. **[P2] 審查子單失敗／取消後，父單沒有收尾事件。**  
   位置：[aos_team_task.py:193](../../../lib/aos_team_task.py)、同檔 240、379 行。  
...
10. **[P2] rm 搬家與更新名冊之間沒有恢復紀錄。**  
    位置：[aos_team.py:225](../../../lib/aos_team.py)。  
