...
1. **工具結果會被「後面的呼叫」改名。**
   - 在哪：[aos_agent_listen_render.py:180](../lib/aos_agent_listen_render.py)、同檔 213；`aos_agent_listen.py:217`。
...
2. **fallback 接受未驗證記憶，新印法會直接崩潰。**
   - 在哪：[aos_agent_listen.py:55](../lib/aos_agent_listen.py)、`aos_agent_listen_render.py:186、192、216`。
...
3. **超長合法數字會讓 CLI 崩潰。**
   - 在哪：[aos_agent_cli.py:98](../lib/aos_agent_cli.py)。
...
4. **封存分組與排序無法完整還原收件順序。**
   - 在哪：[aos_agent_listen_render.py:76](../lib/aos_agent_listen_render.py)、同檔 83。
...
5. **不合理的封存時間戳會讓回話印不出來。**
   - 在哪：[aos_agent_listen_render.py:97](../lib/aos_agent_listen_render.py)。
...
6. **簡化呼叫行不保證只有一行。**
   - 在哪：[aos_agent_listen_render.py:128](../lib/aos_agent_listen_render.py)。
...
7. **follow 真程序測試可能永久卡住。**
   - 在哪：[test_agent_listen_tweak.py:219](../lib/test/test_agent_listen_tweak.py)、同檔 237。
...
1. **重複問句的時間應標明只是推定。**
   - 在哪：[aos_agent_listen_render.py:94](../lib/aos_agent_listen_render.py)、規範 `cli-listen.md:32`。
...
2. **封存掃描應排除特殊檔，並控制成本。**
   - 在哪：[aos_agent_listen_render.py:61](../lib/aos_agent_listen_render.py)、同檔 71。
...
3. **补齊重要測試空缺。**
   - 在哪：[test_agent_listen_tweak.py:93](../lib/test/test_agent_listen_tweak.py) 起。
...
4. **help 與規範有兩處容易過度承諾。**
   - 在哪：[aos_agent_cli.py:24](../lib/aos_agent_cli.py)、`cli-listen.md:38`。
