...
這裡稱為「LLM cpu」的 worker，**沒有登記成 `K/cpus/` 裡的一顆普通 cpu**。module 的排程函式在 kernel tick 行程內執行；模型呼叫則在它啟動的背景 worker 行程中執行。[派工實作](../../proto4-5/llm_cpu_tick.py:192)、[kernel 呼叫順序](../../proto4-3/aos_kernel_tick.py:41)
...
來源：[proto5 agent 格式](../spec/agent.md:15)、[三格規範](../spec/aos-agent.md:22)、[engine 與請求規範](../../proto5.1/spec/aos-llm-ask.md)、[函式庫說明](../lib/README.md:243)、[現有同步 think](../lib/aos_agent.py:215)。
...
| 檔案 | 實際職責 |
|---|---|
| [aos-llm](../../proto4-5/aos-llm:1) | 薄入口，呼叫 `aos_llm.main()` |
| [aos_llm.py](../../proto4-5/aos_llm.py:89) | endpoint／request 驗證、HTTP、模型預檢、result 正規化 |
| [aos_llm_cli.py](../../proto4-5/aos_llm_cli.py:15) | endpoint 檔選取、REQ／OUT 檔案與 stdin/stdout、CLI 退出碼 |
| [llm_cpu_worker.py](../../proto4-5/llm_cpu_worker.py:37) | 第二層的使用者：直接 import `aos_llm.call()`，沒有再啟動 `aos-llm` CLI |
...
來源：[第一層驗證](../../proto4-5/aos_llm.py:45)、[CLI 選 endpoint](../../proto4-5/aos_llm_cli.py:20)、[第二層載設定](../../proto4-5/llm_cpu_home.py:149)、[第二層 endpoint 驗證](../../proto4-5/llm_cpu_tick.py:127)。
...
來源：[headers](../../proto4-5/aos_llm.py:73)、[worker 啟動](../../proto4-5/llm_cpu_tick.py:209)。
...
單一 endpoint 檔加 `#name` 會被拒絕。OUT 不會自動建立父目錄。第一層沒有 `--wait`、排隊、`--dry-run` 或重試旗標。[CLI 實作](../../proto4-5/aos_llm_cli.py:20)
...
來源：[請求驗證及 body 組裝](../../proto4-5/aos_llm.py:95)。`params.messages` 覆蓋行為另以純記憶體 `_open` 替身確認，未連網。
...
`models()` 要求回應物件含 `data` 陣列；只收其中為物件且 `id` 是字串的項目，其他項目略過。[預檢與 POST](../../proto4-5/aos_llm.py:125)、[models](../../proto4-5/aos_llm.py:204)
...
`tool_calls` **不提升到 result 頂層**，保留在 `raw.choices[0].message.tool_calls`。回覆有 `content:null` 與 tool calls 可以成功；若完全缺 `content`，第一層會回 `bad_response`。[usage／錯誤 envelope](../../proto4-5/aos_llm.py:11)、[成功回覆擷取](../../proto4-5/aos_llm.py:174)
...
第一層不因 `retryable:true` 自動重試。另外，函式不是對任意畸形回覆都保證回 result：例如非空非物件的 `prompt_tokens_details` 會讓 `_usage()` 拋 `AttributeError`；直接 CLI 沒有總括捕捉，第二層 worker 才會把這類例外包成 `internal`。此例外路徑已用純記憶體替身確認。[CLI 邊界](../../proto4-5/aos_llm_cli.py:93)、[worker 例外封裝](../../proto4-5/llm_cpu_worker.py:37)
...
| 檔案 | 職責 |
|---|---|
| [llm_cpu.py](../../proto4-5/llm_cpu.py:11) | `init`／`tick`／`submit`／`ls`／內部 `worker` CLI |
| [llm_cpu_home.py](../../proto4-5/llm_cpu_home.py:43) | 建家、原子 JSON、submit、endpoint 文件、獨立模式 ls |
| [llm_cpu_request.py](../../proto4-5/llm_cpu_request.py:15) | ID 驗證、請求指紋、查同名單所在位置 |
| [llm_cpu_tick.py](../../proto4-5/llm_cpu_tick.py:231) | 收尾、驗件、排序、容量、派工、snapshot／log |
| [llm_cpu_worker.py](../../proto4-5/llm_cpu_worker.py:37) | 在背景同步問模型、寫結果與 usage |
| [llm_cpu_module.py](../../proto4-5/llm_cpu_module.py:13) | 接 kernel 的四個 hook；syscall、兩段等待、module 摘要 |
| [llm_cpu_manage.py](../../proto4-5/llm_cpu_manage.py:31) | `aos-kernel llm ls/rm`，直接查檔／殺 worker／刪單 |
...
其中 `endpoints` 的 value 是**各 endpoint 正在 running 的件數**，不是 endpoint 設定物件。真正佇列／執行狀態由各資料夾中的檔案決定。[建家](../../proto4-5/llm_cpu_home.py:43)、[snapshot](../../proto4-5/llm_cpu_tick.py:242)
...
排程 ID 來自 `--name`／module ticket 的 `id`／未指定時的 `time.time_ns()`。限制是 `[A-Za-z0-9._-]+`，但額外禁止 `"."`、`".."`、以及以 `.tmp` 結尾。[ID 驗證](../../proto4-5/llm_cpu_request.py:7)、[request 驗證](../../proto4-5/llm_cpu_tick.py:110)
...
獨立模式 CLI `submit` 是**複製原始 bytes**，此時不解析、不驗 JSON，因此可以成功投入壞 JSON；下一 tick 才退件。module 使用 `submit_object()`，先驗 request，才序列化投遞。[兩種 submit](../../proto4-5/llm_cpu_home.py:74)
...
來源：[worker 寫結果](../../proto4-5/llm_cpu_worker.py:37)、[scheduler 錯誤 result](../../proto4-5/llm_cpu_tick.py:15)。
...
不是每個終局都會有 usage：**只有 worker 的 `_write_usage()` 會寫**；tick 自行退件、`worker_died`、spawn 失敗或 watchdog timeout 不會補 usage。[usage 實作](../../proto4-5/llm_cpu_worker.py:16)
...
來源：[tick 主流程](../../proto4-5/llm_cpu_tick.py:231)。
...
spawn 失敗會立即寫 `spawn` 錯誤 result、將 request 移入 done。worker 不負責把 request 移入 done。[dispatch](../../proto4-5/llm_cpu_tick.py:192)、[worker](../../proto4-5/llm_cpu_worker.py:37)

**2.7 收尾、逾時、重試與撤單**

| 機制 | 判定／動作 | 所在檔 |
|---|---|---|
| 已有 result | 只判 `result_path.exists()`，不解析 result；request 移 done | [tick:72](../../proto4-5/llm_cpu_tick.py:72) |
| running metadata 壞 | 寫 `worker_died`，移 done | 同上 |
| PID 不存在／無有效 PID | `os.kill(pid,0)` 判存活；沒有結果則 `worker_died`，移 done | [tick:51](../../proto4-5/llm_cpu_tick.py:51) |
| HTTP timeout | 第一層 urllib timeout；worker 寫 timeout result | [aos_llm:154](../../proto4-5/aos_llm.py:154) |
| scheduler hard timeout | `time.time()-started > timeout_ms+5000ms`；SIGTERM **單一 PID**，寫 timeout，移 done | [tick:98](../../proto4-5/llm_cpu_tick.py:98) |
| hard timeout 的等待／升級 | 不等待退出，不升 SIGKILL | 同上 |
| 自動重試 | 第一層、worker、scheduler 都沒有；不消費 `retryable` 來重送 | 上述呼叫鏈 |
| `llm rm` 刪 queued | 直接刪 request/result 等同名檔 | [manage:107](../../proto4-5/llm_cpu_manage.py:107) |
| `llm rm` 刪 running | 先讀 PID；對 process group SIGTERM 等 1 秒，再 SIGKILL 等 1 秒 | [manage:85](../../proto4-5/llm_cpu_manage.py:85) |
| `llm rm` 清除範圍 | queued、running、done request、result 四处 | [manage:112](../../proto4-5/llm_cpu_manage.py:112) |
...
- `llm rm` 遇到 running 記錄沒有可殺 PID 時退 1，保留檔案；既有測試有覆蓋。[管理測試](../../proto4-5/test/test_module_manage.py:115)
...
它會忽略物件 key 順序與序列化空白，**沒有補入 default endpoint、priority 等預設值做語意正規化**；也不把 endpoint 設定內容納入 fingerprint。[fingerprint 與搜尋次序](../../proto4-5/llm_cpu_request.py:21)
...
來源：[CLI](../../proto4-5/llm_cpu.py:35)、[module hooks](../../proto4-5/llm_cpu_module.py:37)。
...
不是模型 response。[module CLI](../../proto4-5/llm_cpu_module.py:98)、[handle](../../proto4-5/llm_cpu_module.py:37)
...
`--wait` 不會幫 kernel 跑 tick。`--json` 輸出完整 result 後仍會另外印一行「結果檔：…」，所以 stdout **不是只有一個 JSON 文件**。預設只印 `result.text`；tool-call-only result 的 text=null 時會印空行。[等待實作](../../proto4-5/llm_cpu_module.py:174)
...
來源：[kernel 先 syscall 後 module tick](../../proto4-3/aos_kernel_tick.py:48)、[建家位置](../../proto4-5/llm_cpu_module.py:21)、[撤單](../../proto4-5/llm_cpu_module.py:154)、[syscall 未先 claim](../../proto4-3/aos_kernel_syscall.py:86)。
...
以上是可由寫入順序直接辨識的窗口，不是本次故障注入測試的結果。[dispatch 與 PID 寫回](../../proto4-5/llm_cpu_tick.py:200)、[worker 寫入順序](../../proto4-5/llm_cpu_worker.py:54)
...
`aos-agent`、`aos-user` 都只有兩行 shell，分別 `exec python3` 到 `aos_agent_cli.py`、`aos_user_cli.py`。agent CLI 引入 `proto4-6/aos_py.py`，作為呼叫 aos-exec／kernel 的 helper。[agent 入口](../../proto4-7/aos-agent:1)、[CLI import](../../proto4-7/aos_agent_cli.py:9)
...
`new` 建 agent.json、空 messages、信箱、outbox、echo/sh 工具與 inst；**不預先建立 state.json**。state 不存在時由 agent 使用預設值。[建立 agent](../../proto4-7/aos_user_cli.py:52)
...
agent 只檢查 K 是目錄，不在這裡驗證 `config.json` 或 llm module 是否存在；更深的問題在 submit 時才顯現。[設定驗證](../../proto4-7/state_machine.py:25)
...
載入時是「預設 dict 加上檔案內容」。明確驗證只有 `state` 合法、`epoch` 是非負整數且不是 bool；其餘欄位沒有逐一嚴格驗型，未知 key 也會保留。[defaults／load_state](../../proto4-7/state_machine.py:14)、[outbox 编號](../../proto4-7/mailbox.py:11)

`--reset`：讀舊 state，寫全新 defaults，只有 epoch=舊值+1；不刪 messages、outbox、舊 LLM 請求或結果。舊 state 讀壞時 reset 也會失敗。[reset](../../proto4-7/aos_agent_cli.py:26)
...
來源：[idle](../../proto4-7/state_machine.py:94)、[ask](../../proto4-7/state_machine.py:119)、[wait](../../proto4-7/state_machine.py:172)、[act](../../proto4-7/state_machine.py:200)。
...
來源：[ask](../../proto4-7/state_machine.py:119)、[llm_submit](../../proto4-6/aos_py.py:150)、[aos_py.call](../../proto4-6/aos_py.py:56)。
...
它只看結果檔，不讀 syscall 回單、不查 worker PID、不查 queued/running 狀態、不呼叫 `aos_py.wait_for()`，也不 sleep 等結果。[wait 原碼](../../proto4-7/state_machine.py:153)
...
有 calls 時，一格內按順序同步跑完所有 calls。最後 `_save()` **先重寫 messages，再重寫 state**。result 留在 K，不消費、不改名、不刪。[act／save](../../proto4-7/state_machine.py:200)
...
`_record_error()` 沒有清 `request`；錯誤後的 idle state 可能仍顯示上一個結果路徑。下一次成功 submit 才覆寫。[錯誤記錄](../../proto4-7/state_machine.py:78)、[idle 重試條件](../../proto4-7/state_machine.py:108)
...
proto4-7 的單檔 JSON 寫法是 `.tmp + os.replace`，沒有 fsync，也沒有跨檔 transaction。它與 proto5 文件中描述的 `think/act` 尾訊息自癒機制不能視為同一套保障。[原子寫 helper](../../proto4-7/common.py:15)、[收信先搬 read](../../proto4-7/mailbox.py:38)、[act 順序](../../proto4-7/state_machine.py:217)
...
来源：[工具宣告與修復](../../proto4-7/agent_tools.py:14)、[run_tool](../../proto4-7/agent_tools.py:105)、[aos-exec timeout](../../proto4-3/aos_exec.py:32)。

**cwd 有一個重要細節**：通用工具是普通檔案目標，aos-exec 把 cwd 設為 run 所在的 `A/tools/<name>/`。內建 `sh/run` 自己算出 agent 根並 `cd`，所以它才是在 A 裡執行 shell。[普通檔案 cwd](../../proto4-3/aos_exec.py:88)、[內建 sh](../../proto4-7/aos_user_cli.py:35)
...
它不是完整解析 XML／function 標籤，也不會從 `<function=foo>` 的標籤本身取工具名；仍要求 JSON object 裡有 `name`。[修復實作](../../proto4-7/agent_tools.py:43)
...
來源：[mailbox](../../proto4-7/mailbox.py:38)、[listen](../../proto4-7/aos_user_cli.py:110)。
...
aos-user：正常 0；已捕捉的 AgentError／OSError／ValueError 為 1；argparse 用法錯 2。[agent CLI](../../proto4-7/aos_agent_cli.py:18)、[user CLI](../../proto4-7/aos_user_cli.py:179)
...
來源：[init](../../proto4-3/aos_kernel_init.py:24)、[loader](../../proto4-3/aos_kernel_module.py:16)。
...
`h` 是 `KHome`，含 `.dir`、`.syscalls`、`.syscalls_done`、`.procs`、`.cpus` 等路徑；cfg 是讀出的設定；st 是當回合 kernel state。[KHome](../../proto4-3/aos_kernel.py:61)
...
找不到則使用 cwd。[module CLI 分派](../../proto4-3/aos_kernel.py:171)
...
LLM request 可在第 4 步排入、第 5 步同回合派工。即使 daemon 上普通 cpu 尚未 ready，module hook 仍會被執行。[tick](../../proto4-3/aos_kernel_tick.py:41)
...
來源：[handle_syscalls](../../proto4-3/aos_kernel_syscall.py:72)、[回音／刪單](../../proto4-3/aos_kernel_syscall.py:191)。
...
來源：[rm CLI](../../proto4-3/aos_kernel_syscall.py:18)、[llm 等回音](../../proto4-5/llm_cpu_module.py:233)。
...
來源：[aos-run 回報](../../proto4-3/aos_run.py:85)、[daemon 解析](../../proto4-3/aos_daemon_entry.py:94)、[kernel 讀狀態](../../proto4-3/aos_kernel_tick.py:62)、[scheduler](../../proto4-3/aos_kernel_schedule.py:40)。
...
101 讓出的是**後續 cpu 執行機會**，不是將一個仍阻塞在模型呼叫中的 agent 行程移走。[換人／還原 waiting](../../proto4-3/aos_kernel_schedule.py:94)
...
| # | 文件／註解說法 | 程式事實 | 位置 |
|---:|---|---|---|
| 1 | proto5 aos-agent「程式還沒寫」 | 工作樹已有 lib/CLI；think 直接同步 call | [README:16](../README.md:16)；[aos_agent:215](../lib/aos_agent.py:215) |
| 2 | proto5 lib 列「五支模組」，llm_ask 說 agent「之後」import | 已有第六支 aos_agent，且正在 import／使用 | [lib README:1](../lib/README.md:1)、[244](../lib/README.md:244)；[aos_agent:18](../lib/aos_agent.py:18) |
| 3 | proto4-5「沒有取消」 | 已有 `aos-kernel llm rm`，可殺 running worker 並刪單；另有收件逾時撤 syscall | [README:181](../../proto4-5/README.md:181)；[manage:85](../../proto4-5/llm_cpu_manage.py:85) |
| 4 | 「每一發另 append usage.jsonl」 | 只有 worker 寫 usage；scheduler 退件、spawn、worker_died、hard timeout 不寫 | [README:157](../../proto4-5/README.md:157)；[worker:55](../../proto4-5/llm_cpu_worker.py:55)、[tick:35](../../proto4-5/llm_cpu_tick.py:35) |
| 5 | result 說明把 notes 列為通用欄位 | scheduler 自產錯誤 result 沒有 notes | [README:66](../../proto4-5/README.md:66)；[tick:23](../../proto4-5/llm_cpu_tick.py:23) |
| 6 | 「連線及逾時會標 retryable:true」 | 第一層 HTTP timeout 是 true；scheduler hard timeout 使用預設 false | [README:177](../../proto4-5/README.md:177)；[tick:15](../../proto4-5/llm_cpu_tick.py:15)、[104](../../proto4-5/llm_cpu_tick.py:104) |
| 7 | syscall 還在就可說「沒送出去」 | 原單沒有先 rename claim；kernel 可能已讀入但未 unlink，CLI 刪單成功不能證明未送出 | [README:120](../../proto4-5/README.md:120)；[module:154](../../proto4-5/llm_cpu_module.py:154)、[syscall:86](../../proto4-3/aos_kernel_syscall.py:86) |
| 8 | kernel 沒活著就撤掉、不排隊 | CLI 沒有直接判 kernel 存活；它靠等回音逾時推斷。同名同內容分支甚至不投 syscall，也不撤已有單 | [README:118](../../proto4-5/README.md:118)；[module:137](../../protos4-5/llm_cpu_module.py:137) |
| 9 | `call()` docstring：「永遠以 v1 result 回報成敗」 | 畸形 usage detail 可拋 AttributeError；直接 CLI 未總括捕捉，worker 才包 internal | [aos_llm:89](../../proto4-5/aos_llm.py:89)、[11](../../proto4-5/aos_llm.py:11)；[worker:50](../../proto4-5/llm_cpu_worker.py:50) |
| 10 | 失敗 stderr「一行」 | msg 可直接包含 HTTP body 的換行，print 前沒有壓成單行 | [README:62](../../proto4-5/README.md:62)；[aos_llm:156](../../proto4-5/aos_llm.py:156)、[CLI:119](../../proto4-5/aos_llm_cli.py:119) |
| 11 | agent「連錯五次」stuck | errors 在成功模型／工具輪不清零；實際是同一題期間累計到 5，未要求連續失敗 | [README:27](../../proto4-7/README.md:27)；[state_machine:78](../../proto4-7/state_machine.py:78)、[195](../../proto4-7/state_machine.py:195) |
| 12 | 「每題格數」上限 | step 只在 ask 自增；idle/wait/act 不加，故是 ask 次數上限 | [README:8](../../proto4-7/README.md:8)、[26](../../proto4-7/README.md:26)；[state_machine:119](../../proto4-7/state_machine.py:119) |
| 13 | 收「最舊」的一封信 | 實際按檔名排序，不看 mtime／信件 time；aos-user 的 timestamp 檔名使一般情況看起來是時間序 | [README:25](../../proto4-7/README.md:25)；[mailbox:43](../../proto4-7/mailbox.py:43) |
| 14 | 工具名對不上就算模型錯誤 | 只適用文字修復；正式 tool_call 的未知工具變成 tool content「沒有這個工具」，不加 errors | [README:91](../../proto4-7/README.md:91)；[tools:86](../../proto4-7/agent_tools.py:86)、[110](../../proto4-7/agent_tools.py:110) |
| 15 | 所有改寫 JSON 都先 tmp/rename | agent 自己主要檔案如此；ask 呼叫的 helper `<name>.req.json` 是直接 write_text | [README:19](../../proto4-7/README.md:19)；[aos_py:155](../../proto4-6/aos_py.py:155) |
| 16 | 工具程式註解稱 parameters 一定 object、有 properties | 只補缺省；已有 `type:"array"` 或非 dict properties 不會被改掉 | [tools:29](../../proto4-7/agent_tools.py:29) |
| 17 | module CLI 第一個參數符合才當 K | 實作掃前兩個參數，支援 action 後接 K | [kernel 文件:84](../../proto4-3/docs/kernel.md:84)；[kernel:175](../../proto4-3/aos_kernel.py:175) |
| 18 | module 載入失敗會留 note | tick／ls 會處理 notes；module CLI 丟掉 loader.notes，可能只顯示「不認得子命令」 | [kernel 文件:65](../../proto4-3/docs/kernel.md:65)；[kernel:186](../../proto4-3/aos_kernel.py:186) |
| 19 | kernel tick 退出碼「一律 0；不是家=1」 | 帶參數明確退 2；主 tick 也沒有捕捉所有自身 I/O 例外的總括邊界 | [kernel 文件:57](../../proto4-3/docs/kernel.md:57)；[tick:140](../../proto4-3/aos_kernel_tick.py:140)、[41](../../proto4-3/aos_kernel_tick.py:41) |
| 20 | 排程段括號仍寫「v1 的行程不會自己結束」 | 已有 done_exit，預設 100，搬入 procs/done | [kernel 文件:127](../../proto4-3/docs/kernel.md:127)；[schedule:42](../../proto4-3/aos_kernel_schedule.py:42) |

另有幾項屬於**文件未完整交代，而非直接相反**：

| 主題 | 程式中的補充事實 | 來源 |
|---|---|---|
| 第一／第二層 endpoint 不同要求 | 第一層可省 timeout/max_concurrent；第二層兩個都必填 | [第一層](../../proto4-5/aos_llm.py:58)、[第二層](../../proto4-5/llm_cpu_tick.py:135) |
| 第一層 params 的覆蓋能力 | 只強制保護 model/stream；messages 可被 params 覆蓋 | [body 組裝](../../proto4-5/aos_llm.py:140) |
| `--wait --json` stdout | 完整 JSON之外另有結果路徑文字行 | [wait_result](../../proto4-5/llm_cpu_module.py:184) |
| module 衝突 | 不拒絕重名／重疊 OPS，分派取前者 | [CLI](../../proto4-3/aos_kernel.py:187)、[syscall](../../proto4-3/aos_kernel_syscall.py:105) |
| syscall wire schema | 實作為同名檔案關聯、固定 `{ok,msg}`，不是包含完整結果的通用回覆 | [syscall](../../proto4-3/aos_kernel_syscall.py:191) |
| agent 的 101 | kernel 不讀結果檔；只讓位、再輪到時重新跑 agent | [schedule](../../proto4-3/aos_kernel_schedule.py:68) |
...
|---|---:|---|
| [test_aos_llm.py](../../proto4-5/test/test_aos_llm.py:15) | 16 | endpoint 三種選法、result 全欄位、tools/tool_choice、HTTP500、禁止 model、stdin/stdout、missing key、timeout 覆寫、strict 預檢／fallback |
| [test_home.py](../../proto4-5/test/test_home.py:11) | 12 | 建家、submit、撞名、壞 request、process 拒絕、壞 endpoints tick 仍 0、ls、aos-exec 一次一 tick |
| [test_worker.py](../../proto4-5/test/test_worker.py:8) | 9 | success/usage、HTTP500、壞 JSON、model mismatch、alias、missing key、connection refused、固定 model |
| [test_schedule.py](../../proto4-5/test/test_schedule.py:10) | 7 | priority、mtime、容量、跨 endpoint、worker 死亡、HTTP timeout、hard timeout |
| [test_module.py](../../proto4-5/test/test_module.py:19) | 14 | 首 tick 建家、syscall 收件、wait/plain/json、两種收件逾時分支、結果逾時、同名同內容冪等／撞名 |
| [test_module_manage.py](../../proto4-5/test/test_module_manage.py:19) | 6 | init 提示、placeholder 警告、ls、rm、殺 running worker、無 PID 拒絕 |
| 合計 | **64** | 使用假 OpenAI server、CLI 與暫存家 |

`_fake_openai.py` 提供 `/models`、echo、延遲、500、壞 JSON、model mismatch 等情境；`_util.py` 管理 fake server、暫存家、CLI、結果輪詢與清理。[fake server](../../proto4-5/test/_fake_openai.py:9)、[共用 fixture](../../proto4-5/test/_util.py:19)

| proto4-7 測試檔 | 方法數 | 涵蓋內容 |
|---|---:|---|
| [test_agent.py](../../proto4-7/test/test_agent.py:18) | 16 | 四格、req 組裝、一信一題、陣列信、壞信、step 上限、精確 600 次、第五次錯誤、epoch、20 次重試、stuck 清除、退出碼、submit 失敗 |
| [test_tools.py](../../proto4-7/test/test_tools.py:17) | 9 | schema 補預設、正式／文字 call、普通文字、救回失敗、缺工具／壞參數、非零退出、截斷 |
| [test_user.py](../../proto4-7/test/test_user.py:18) | 6 | new、內建工具、say、listen、status、talk、reset |
| 合計 | **31** | 假 kernel、人工寫 result、本機 shell 工具 |

proto4-7 的 fake kernel 是把 helper request 複製到假 `K/llm/requests/`，測試再自行寫 result；這組測試**不是實際 kernel module→worker→HTTP 的整條端到端測試**。[fake kernel](../../proto4-7/test/test_agent.py:26)

kernel 的相關測試明確涵蓋「101 無競爭時留 cpu」「有人排隊時讓位」「waiting 在 ls 顯示」「101 不累計一般錯誤」。[退出碼政策測試](../../proto4-3/test/test_kernel_exit.py:18)
