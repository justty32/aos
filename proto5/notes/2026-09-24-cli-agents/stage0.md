...
照使用者拍板的五條把階 0 做出來。**`lib/`、`spec/`、`cli/` 一行沒改。**

## 做了什麼

| 件 | 檔 |
|---|---|
| 範本（放 `templates/`：給人複製去用的） | [templates/cli-agents/](../../templates/cli-agents/README.md)：`kernel2.json`、`claude-job.json`、`claude-next.json`（分岔）、`codex-review.json`、`codex-review-next.json`（分岔）＋README（旗標為什麼、成功怎麼判） |
| 教程 | [tutorials/07-cli-agents.md](../../tutorials/07-cli-agents.md)，開頭就是「三個洞」；tutorials／proto5 README 各加一列 |
| 一條龍腳本 | [stage0-run.sh](stage0-run.sh)：`bash stage0-run.sh [W] [--with-claude] [--no-codex]`；任何一步失敗就停、不再放單、退非 0；「成功」＝回音與輸出兩層都過才推 session |
| 審查 | [review2-task.md](review2-task.md)／[review2-astra.md](review2-astra.md) |

範本的旗標：claude `-p --output-format json --model sonnet --max-turns 8 --max-budget-usd 0.50 --safe-mode --restricted --permission-mode acceptEdits --permission-prompts none --disallowedTools "Bash(git push:*)"`；
...
## 五條驗證（cli-facts 末尾）：四條真跑、一條照指示跳過
...
2. **訂閱下 `--max-budget-usd`**：**有用**。設 0.001 → `is_error: true`、`subtype: error_max_budget_usd`、`terminal_reason: budget_exhausted`、退 1；但它是呼叫**完**才算（那次照算 0.019 美元），擋不住第一次呼叫。`total_cost_usd` 訂閱下也印（照定價算，不是真的扣錢）。
...
## 一條龍真跑（節錄，第一版腳本 `stage0-run.sh … --with-claude`；審查後的版本再跑了 `--with-claude --no-codex`，兩張 claude 單都成功）
...
（answer.md：三條，ls:26 符號連結指到根目錄外、ls:27 檔名含換行、_common.py:125 權限不足誤報 NotFound）
...
codex 那三條是對 `tools/base/ls` 的真意見，**這次沒修**（不在範圍），留給 tools 那條線。
...
| 審查指出 `--permission-prompts none` 擋不住唯讀指令與部分檔案指令，「不准跑任何指令」不成立 | claude 範本加 `--restricted`（真跑確認照常回話）；「加 `Bash(make test)`」改寫成「等於准它跑工作區裡任意程式」 |
...
claude 真跑 **6 次**（直接 2：stdin／json、預算；經 kernel 4：兩輪各 c1、c2），每次都只回 `ok`，照定價算共約 0.13 美元。
codex 真跑 **6 次**（直接 4：只回 ok、分岔試沙盒 ×2、逾時砍；經 kernel 1：審 `tools/base/ls`，約 14 萬 input token；F 審查 1）。
...
| 4 | claude 範本預設 `--restricted`（沒有會跑程式的工具，只准改工作區的檔）；要跑測試得拿掉它並逐條加 `--allowedTools`，等於准它跑工作區任意程式 | **是** |
| 5 | 範本的 `--max-budget-usd 0.50`、`--max-turns 8`、逾時 30 分鐘 | **照這組**，用一陣子再調 |

**2026-09-24 使用者裁決：預設不帶，其他四題照預設。**（claude 範本不帶 `--restricted`：讓 claude 能跑工作區裡的程式；要更保守自己在單子加 `--restricted`）

## 審查（F）

[review2-astra.md](review2-astra.md)：必修 7 條全改——claude 加 `--restricted`（1）；`make test` 白名單的說法改寫（2）；腳本與教程的「成功」改成回音＋輸出兩層、c2 成功後也推 session（3）；腳本失敗即停、退非 0（4）；教程改用 `$W/cli-jobs`、`$W/cli-ws`，收工只刪自己建的、確認 `stopped` 才刪（5）；路徑限英數與 `/ . _ -`，腳本會擋（6）；取消的說法改成「回音只代表受理、5 秒後 TERM、脫離那組的子孫砍不到」，並分清 `rm` 印名字、`Removed` 是 add 單的回音（7）。
建議 4 條也照改：教程開頭要先回 repo 根目錄、`auth.json` 要在；`clear` 會連代理一起清、只限三顆工作 cpu；符號連結風險與「只隔開家目錄那份設定」寫進教程；「五條驗證」改成「四條真跑、一條跳過」、codex 花費寫 token 數不說「很小」。
