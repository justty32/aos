...
| 檔 | 是什麼 | 要換的字 |
|---|---|---|
| [kernel2.json](kernel2.json) | 花錢的 kernel 家 K2 的池表：`claude` 池 1 顆、`codex` 池 2 顆（池名別跟 K 的撞：daemon 那邊同名會被拒）| `@W@`＝工作目錄 |
...
| claude、codex 兩池的環境 `$opt: clear` | 只留 `PATH`、`HOME`、`LANG`（codex 再加 `CODEX_HOME`）：擋掉剛好在環境裡的 `ANTHROPIC_API_KEY`／`OPENAI_API_KEY`（免得改走付費 API），也擋掉從 Claude Code 裡開 daemon 時帶進來的 `CLAUDECODE`、`CLAUDE_CODE_*`。代價：代理（`HTTPS_PROXY`）、自訂憑證路徑之類也一起清掉，你的環境要靠它們才連得上，就在 `$val` 裡逐項補回 |
