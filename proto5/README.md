...
| [spec/aos-agent.md](spec/aos-agent.md) | `aos-agent [dir]` 走一格：`state.json` 的 `input`（輸入接進記憶、讀完 rename）／`waits`（盯哪些檔、`exists`／`mtime`／`consume`／`any`／`all`）、四格各做什麼、退出碼 0／101／1／2；跑工具（aos-tool-exec）與逾時還沒想好 | **草稿**；程式還沒寫 |
