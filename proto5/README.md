...
| [spec/aos-agent.md](spec/aos-agent.md) | `aos-agent [dir]`：先看 `waits` 這道門（動態等待表、`exists`／`mtime`／`consume`／`any`／`all`、逾時），門開了走一格（`idle` 收 `input`／`returns`、`think` 問、`act` 跑完所有工具接 `tool` 訊息）；三格狀態、退出碼 0／101／1／2；非同步工具之後再做 | **草稿**；程式還沒寫 |
