...
| [spec/aos-agent.md](spec/aos-agent.md) | `aos-agent [dir]` 做什麼：先判 `waits` 這道門（到了就劃掉、還有剩就 101），門開了走一格（`idle` 收 `input`、`think` 問、`act` 跑完所有工具接 `tool` 訊息）；退出碼 0／101／1／2；非同步工具、逾時之後再做 | **草稿**；程式還沒寫 |
