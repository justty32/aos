...
> 2026-09-23 重架構第二份；同日照 astra 兩輪審查改過（第一輪 K／X／R，第二輪 K2／X2／R2）。
...
**`K/state.json` 是唯一的帳本**：每個決定都是一次原子寫，其他檔都是從它放出去的。
...
| kernel cpu | `info.cpus` 裡 pool 標 `kernel` 的那一顆 exec cpu，恰好一顆，專門拿來排 tick；一般工作不准進去 |
...
| 殘格 | 舊鏈排在隊上、現在才被跑到的 tick。它 args 裡的 chain 對不上 state 的，所以什麼都不做就退出（自滅） |
| syscall | 借作業系統的講法；在這裡只是「往 `K/requests/` 放一個 JSON 檔」（`add`／`rm`／`stop`）。下一格會讀它，回音寫在 `K/responses/` |
| ack | 收件者說「這份回音我拿走了」的那張單（範式 §3.3）；放了對方才刪回音。kernel 收 cpu 的回音要放 ack 給 cpu；別人收 kernel 的回音要放 ack 給 kernel |
| 帳本 | `K/state.json`。kernel 所有的記憶：鏈、階段、每顆 cpu 手上的工作、每個行程的紀錄與計數、還沒放出去的東西。一次原子寫 |
| 出貨箱（outbox） | 帳本裡三張「記了、還沒放出去」的清單：`acks`（欠 cpu 的 ack）、`replies`（欠客戶的回音）、`stops`（欠 cpu 的 stop）。先記進帳本、再放檔、放完清掉；崩在中間下一格補放 |
...
| 狀態（`status`） | 一個行程現在在哪：`queued` 排隊中／`running` 在某顆 cpu 上／`done` 做完了／`bad` 被退件。後兩種只是留給人看，`rm` 能刪 |
| 佇列（`queue`） | 等著被派上 cpu 的行程名單；已經在 cpu 上跑的不在裡面 |
| boot | 開機：先叫舊的 kernel cpu 停、等它死透，發一個新鏈 id、拉起所有 cpu、把第 1 格放進 kernel cpu。boot 自己不跑格 |
...
| `req`／`proc` | `req`＝派給這顆 cpu、還沒收到回音的 request 檔名；`proc`＝那則是哪個行程的。**一顆 cpu 忙不忙只看 `req` 是不是 `null`**，kernel 不去讀 cpu 自己的 state |
| `discard` | 那個行程已經被 `rm` 了，但工作還在 cpu 上跑；回音到了直接丟掉、不計數 |
| 在途 | 已經派出去、還沒收到回音的工作（`req` 非 `null` 的那些）。`stopping` 要等它們全部收完才真的停 |
| 冪等 | 同一步做一次跟做兩次結果一樣。崩了重跑才安全——帳本＋出貨箱就是為這個 |
| EEXIST（在這裡） | `link` 到已經存在的名字會失敗。kernel 把它當「已經放過了」的訊號，照樣往下走，不當錯誤 |
| 先放後記 | 接鏈的順序：**先**把下一格放進 kernel cpu 的 `requests/`、**再**寫帳本。崩在中間下一格照跑，鏈不會斷 |
| 先記後放 | 派工、回音、ack、stop 的順序，跟接鏈剛好相反：**先**寫帳本、**再**放檔。崩在中間最多「記了沒放」、下一格補放，絕不會同一個行程被派到兩顆 cpu |
...
kernel 對它們也是外人，只在**它們啟動前**替它們寫 `info.json` 與 `inst.json`（初始化，不算動別人的家）。
...
```json
{
  ...
  "daemon": "/abs/D",
  "cpus": {"k": {"pool": "kernel"}, "0": {}, "1": {}, "llm": {"pool": "llm"}},
  "tick_ms": 1000,
  "interval_ms": 1000,
  "timeout_ms": 0,
  "done_exit": 100,
...
| `cpus` | 物件 | 必填 | key 是 cpu 名（也是 `cpus/<name>/` 的資料夾名、給 daemon 的孩子名）；值 `{"pool": 字串}`，沒寫＝`"default"`。**恰好一顆** pool 是 `kernel`（少於或多於一顆＝`FieldTypeMismatch`），tick 鏈跑在它上面，一般行程不准進這個池 |
...
整份解指示詞，中心是 K，不提供 `$opt`。**頂層必須是字面物件**（頂層整份 `$ref` ＝ `FieldTypeMismatch`），
不然 boot 寫進去的 `daemon` 會被引用吃掉。boot 把整份驗完才動任何東西（§6）。
「專門打 LLM 的 cpu 只開一顆」就是開一顆 `{"pool":"llm"}`、把 `aos-llm-call` 那種行程標 `pool: "llm"`。
要幾顆就寫幾顆，佔著沒關係。改 info 之後不用重 boot：tick 每格重讀；多出來的 cpu 下格會拉，
被拿掉的 cpu 若帳本裡還有 `req`，照樣收完那則才忘掉它；池裡沒有 cpu 的行程就一直排隊，不算錯，`ls` 看得出。

### 1.2 `state.json`（帳本）

```json
{
  "chain": "1790000000000000000-4242", "last_seq": 41, "phase": "running",
  ...
  "acks":    [{"cpu": "1", "name": "k-1790000000000000000-4242-39-1.json"}],
...
| `cpus.<name>` | **只有工作 cpu**，kernel cpu 不在這裡。`req`＝派出去、還沒收到回音的 request 檔名，`null`＝閒（**cpu 忙不忙就看這格**）；`proc`＝那則是哪個行程的；`discard`＝那個行程已被 `rm`，回音到了丟掉 |
...
| `acks`／`replies`／`stops` | 出貨箱：記了、還沒放出去的 ack（給哪顆 cpu、哪個名）、回音（給 `K/responses/` 的檔名、id、內容）、stop（給哪顆 cpu）。§3 第 4 步補放 |
...
| 任一 cpu 的 `requests/` | `ack-<chain>-<seq>-<n>.json`、`stop-<chain>.json` | ack、stop |
| daemon 的 `requests/` | `k-<chain>-<seq>-spawn-<c>.json` | 拉 cpu |

全部帶 `chain`，跨 boot 永不重複；同一格對同一顆 cpu 最多派一次，所以格內也不重複。這是範式 §1「名字不重用」在 kernel 這邊的做法。
...
| `ack` | 範式 §3.3 | kernel 是一個家，別人收了 `K/responses/` 的回音要放 ack，處理方式照範式 |
...
**每則 syscall 都是「一次帳本寫入」＋出貨**：判定 → 把結果（行程紀錄、queue、pending、回音）**一次**寫進帳本
（回音進 `replies` 出貨箱）→ 刪原單 → 第 4 步放回音。重掃看到同一份原單（崩在刪原單之前）：
`add` 的 `request` 對得上 `procs.<X>.request` ＝ 已收過，只刪原單；`rm` 的 NAME 已不在 ＝ 已做過，回 `NotFound`
（語意仍是「已經不在」）。

**`rm` 的三種情況**：
- 行程 `queued`／`done`／`bad`：從帳本拿掉（含 queue）。`once` 且 `queued`：它 `pending` 的 add 回 `-32000`／`Removed`。
...
2. `phase=stopped` → 補放 `stops` 出貨箱（§3 第 4 步的方式）、不接鏈、退 0。否則**放檔**：把下一格 `link` 到
   kernel cpu 的 `requests/k-<chain>-<N+1>.json`（`aos-exec`：`target`＝`cli/aos-kernel` 的絕對路徑，是普通檔，
   `args`＝`["tick", K, "--chain", C, "--seq", N+1]`；沒有 timeout）。EEXIST＝上一格已放過（它崩在放檔之後、
   寫帳本之前，這格被重跑）——只有第 N 格會放第 N+1 格，所以 EEXIST 不會是別人。然後**寫帳本** `last_seq=N`。
   **先放後記**：這格之後崩了，下一格照跑。崩在放檔之前＝鏈斷，daemon 的 `restart` 救不了（cpu 沒死、是沒單），
   `ls` 看得出（kernel cpu 的 `requests/` 空、`state.current` null、`last_seq` 不動），人重新 boot。
3. 睡 `tick_ms`（下一格已在隊上，但 kernel cpu 正被這格佔著，睡完退出它才開始；實際週期＝睡＋這格做事的時間）。
4. **出貨**：三張出貨箱每筆放檔（`link`，EEXIST 當已放）、每放完一筆**寫帳本**拿掉。
   順手把 kernel cpu 的 `responses/` 全部 ack 掉（都是舊 tick 的回音，不進 `acks`；`code≠0` 的記 log 一行）。
   這一步跟其他每一步的出貨都是同一套：先記、再放、放完清。
...
7. **daemon**（§5）：偷看 `D/state.json` 的孩子表；info.cpus 有、那邊沒有或 `alive=false` 且不在等重拉的
   → 確定 `cpus/<c>/info.json` 與 `inst.json` 在（沒有就寫），`spawn`（`name`＝c、`restart:true`），等回音。
   死了的 daemon 自己會重拉，這裡不用管；重生那顆手上若有 `req`，它的開機對帳會回 `Interrupted`，
   下格在第 6 步照常收。kernel cpu 也在這裡拉（它死了鏈就斷，這一步只是讓下次 boot 少做事）。
...
9. **停機**（`phase=stopping`）：先把 `queued` 的 `once` 全部拿掉、各回 `-32000`／`Stopping`（進 `replies`）。
   然後所有工作 cpu `req` 為 null、三張出貨箱空、沒有任何 `pending` → `stops`＝所有 cpu（含 kernel cpu）、
   `phase=stopped`、**寫帳本** → 出貨（放 `stop-<chain>.json`）。kernel cpu 收到 stop 就不會再跑已排的下一格，
   那份殘格留著，下次 boot 換了 chain 它會自滅。崩在寫帳本之後、放完之前＝下格（若還跑得到）第 2 步補放；
   跑不到（kernel cpu 已停）就留到下次 boot，boot 第 3 步會把 `stops` 出貨掉再開新鏈。
10. **寫帳本**、append `kernel.log`、退 0。

一格裡 daemon 或磁碟出錯：stderr 一行、退 1、可能只做了一半；帳本＋出貨箱讓下格接得上。

## 4. 回音怎麼判

回音是範式 §4.1 的 aos-exec 回音。先看 cpu 那格的 `discard`：是 → 丟掉、行程紀錄拿掉，不計數。再分兩種：

**`once`**：不管內容是什麼，把回音原樣（`result` 或 `error`）當成 `pending` 那則 add 的回音進 `replies`，
行程紀錄拿掉。`stopped:true` 也原樣交，交件者自己決定。

**反覆**：照順序第一個命中的列做，每列寫清楚三個計數：
...
kernel 對 daemon 只做一件事：`spawn`。看孩子活不活是偷看 `D/state.json`（唯讀），不用 syscall。
`spawn` 是往 `D/requests/` 放單、等 `D/responses/` 同名回音（最多 5000 ms，逾時＝`-32000`／`ReadFailed`，
**逾時不代表沒做**，下格重送是安全的）、放 ack。

| method | kernel 怎麼用 |
|---|---|
| `spawn {"name","target","restart":true}` → `{"pid"}` | `name`＝cpu 名；`target`＝`K/cpus/<c>/inst.json`（argv `aos-cpu K/cpus/<c>`、cwd 那個家、stderr 接 `cpu.log` append）；`restart:true`＝非 0 退出 daemon 自己再拉。同名同 target 已活著＝回它的 pid、不重拉；同名**不同** target＝`NameTaken`——兩個 kernel 共用一個 daemon 又都叫 `k` 會在這裡大聲失敗，不會靜默搶到別人的孩子 |

## 6. 命令列

```sh
aos-kernel init K                     # 建家、預設 info（cpus: k＋0、1、2）；拒絕覆蓋
aos-kernel boot K [--daemon D]        # 見下；不跑整格
aos-kernel tick K --chain C --seq N   # 一格；正常只有鏈自己會叫
aos-kernel add K TARGET [--name NAME] [--once] [--pool P] [--wait-ms N]
aos-kernel rm K NAME
aos-kernel ls K                       # 偷看 K/state.json 與 D/state.json，不放單；鏈斷了也能看
aos-kernel stop K
```

**boot** 只做「交接、拉起來、放第 1 格」，K 的帳本從此只在 kernel cpu 上被改：
1. 驗 D 是 daemon 家、daemon 活著（`D/state.json` 的 pid 非 0）；info 整份驗過（頂層字面物件、恰好一顆 kernel 池）。
2. **交接**：找 kernel 池那顆 cpu；偷看 `D/state.json`，它若活著 → 往它的 `requests/` 放 `stop-boot-<epoch ns>.json`，
   等到 daemon 說它 `alive=false`（最多等 `--wait-ms`，預設 30 秒；逾時＝`AlreadyRunning`，退 1，什麼都沒改）。
   它死了＝沒有任何一格在跑、也不會再有——這才是「舊鏈停了」的證據，不是偷看 `current`。
3. 寫 info.daemon；帳本：`stops` 出貨箱先出貨（上一條鏈崩在停機途中留下的）、`chain`＝新 id、`last_seq`＝0、
   `phase`＝running；`cpus`／`queue`／`procs`／`acks`／`replies` **照舊**（在途 `req`、`pending` 都留著，新鏈接手收）。
4. 每顆 cpu 建家＋`info.json`＋`inst.json`，`spawn` 全部（等回音；kernel cpu 剛被停掉、退 0、daemon 不重拉，
   所以這裡會把它拉回來）。
5. `link` 第 1 格 `k-<chain>-1.json` 到 kernel cpu。退 0。
...
- kernel 不會有兩格同時跑：格都排在同一顆 kernel cpu 上，那顆一次只做一件；boot 不跑格、先把舊 kernel cpu
  停掉等它死透才開新鏈；舊鏈殘格靠 `chain` 自滅。
- 擋不住的：人用手直接 `aos-cpu`／`aos-exec` 跑同一份 inst（規則一之外）；cpu 被 KILL 而子程式還活著
  （範式 §5.3，保證外）。
...
add 拒絕到它跑完；stop 分 stopping／stopped 兩段，在途與 once 的回音收完才停。

## 10. 我自己選的（等你確認）

1. **kernel 跟 daemon 也走資料夾**，不走 pipe——pipe 在 cpu 手上，tick 是孫子。「只有 kernel 能叫 daemon」
   因此是軟性的，換來全系統兩條路一個信封。
2. **tick 用 aos-exec 的普通檔模式**（`target`＝`cli/aos-kernel`、`args` 帶 chain 與 seq），沒有 tick.json：
   chain 釘在 request 裡、不可變，殘格才真的會自滅。
3. **帳本＋出貨箱**：行程紀錄收進 state、`procs/` 資料夾拿掉；ack／回音／stop 先記再放。
4. **boot 的交接是「停掉舊 kernel cpu、等 daemon 說它死了」**，不是偷看 `current`。代價：boot 最慢等一格做完。
5. **`once` 的 add 回音延到跑完才回、內容就是執行結果**；stopping 期間還在排隊的 once 回 `Stopping`。
6. **quantum／waiting／aos_ticks／last_target／獨立的 aos 計數全拿掉**，kind=aos 併進 `fails`——
   agent 先 add 再寫檔不會被「兩次退件」踢掉。
7. **cpu 分池用 `pool` 字串**；kernel 自己的池叫 `kernel`，恰好一顆。
8. **死掉的 cpu 只等 daemon 重拉**，它手上的單靠範式的開機對帳回 `Interrupted`。
9. **kernel 不讀 target 指的檔**，連在不在都不看；一切都是跑起來的回音。
10. **`ls` 不是 syscall**，直接偷看兩份 state。
