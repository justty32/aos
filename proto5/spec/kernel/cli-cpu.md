...
（2026-09-24 proto5-2 池式納入，新節。2026-09-24 one-boot：kernel 池那行拿掉、「kernel 沒在跑」改看 tick 登記。）`cpu add／rm` 只是改 `K/info.json` 的池表（[§1.1](info.md)）——**不放任何單、不用 boot**，
...
- `P` 是 `kernel`＝用法錯（保留名，one-boot 起沒有 kernel 池）。池名、`--dpool` 不合 §1.1 規則＝用法錯 2。負數＝用法錯 2。
- 印 `pool P count A -> B`；kernel 沒在跑（帳本 `phase` 是 running、有 `ticker`、那個 daemon 活著、它登記著這個 kernel，四個缺一）再多印一行
...
- `--count` 必須是正整數（0、負數＝用法錯 2）。`P` 是 `kernel`＝用法錯（保留名）。
...
- 欄位：`want`＝info 現在的 `count`（剛 `cpu add` 完就看得到）；`sent`／`busy`／`idle`（`free` 長度）取帳本；
  `draining`＝已經不是成員、還在做事的號數（顯示時即時算，不等帳本下一格重算）。（第 2 版排第一的 kernel 池那行，one-boot 拿掉；舊 info／帳本留下的 `kernel` 那格也不印。）
...
- `--json` 同樣內容（每池一格，欄位同 [ls `--json` 的 `pools`](cli-ls-json.md)）。
