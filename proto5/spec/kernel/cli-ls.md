...
`dirs` → `stopped` → `daemon` → `cpus` → `recovering` → `stall` → `ok`；讀不到＝`broken`（這時整支退 1、stdout 空，stderr 一行 `aos-kernel: ReadFailed: <那句>`；帳本在 `status()` 讀完後才消失也一樣）；kernel `ok` 時再看 agent：`agents_paused` → `retrying` → `resuming`。
...
- `cpu` 表：info 的 cpu、帳本 `cpus` 的、帳本的 `kcpu` 聯集，**按池分組**（池照第一次出現的順序，同池第二顆起池欄留白；用原值分組，info 裡沒有這顆＝池欄 `?`、空字串池印 `""`，不會跟字面 `-` 併成一組）。`工作`：`閒`／`忙`／`忙（rm）`（行程已被 rm、回音到了丟掉）／kernel cpu 是 `tick`；`行程`＝忙的時候是哪個行程；`daemon`＝D 孩子表裡的 `state`（`running`／`dead`／`killing`…，不在表裡＝`missing`）。
...
- 表下面：每個 `bad` 行程一行 `  <名字> 壞了，看 <路徑>`（路徑規則：target 的 inst 有字面 `stderr` 就指它，agent 就是 `<家>/log/agent.err`；沒有字面 `stderr`（沒寫、寫成指示詞、不是 JSON）就指 target 本身，advice-r1 起不再印「<target> 的 stderr 設定」）。
...
| 欄 | 型別與意思 |
|---|---|
| `_metainfo` | 固定 `{"_type": "aos_kernel_ls", "_version": 1}`。版本內**固定欄位只加鍵、不改名、不刪、不改型別**；要改就升 `_version`。`counts.procs.status` 是資料映射（鍵＝帳本裡出現的狀態），鍵集合隨內容變，不算在這條承諾裡 |
...
| `kernel.settings` | 固定這五個鍵：info 的 `tick_ms`、`interval_ms`、`timeout_ms`、`done_exit`、`bad_after`（省略的已補預設，int） |
...
| `procs[]` | 帳本 `procs` 的順序。`name`；`once`（bool）；`pool`（字串，帳本沒有或不是字串＝`null`）；`status`（字串，帳本沒有或不是字串＝`"unknown"`）；`runs`、`fails`（int，帳本沒有或不是整數＝0）；`pending`（bool：還有沒回的 add）；`target`（絕對路徑字串，帳本沒有＝`null`）；`mark`（agent 的暫停／重試標記 `{code, text}`，code 是 `paused`／`manual_paused`／`both_paused`／`retrying`／`resuming`，沒有＝`null`）；`look`（`bad` 時要看的檔，否則 `null`） |
