...
（2026-09-24 proto5-2 池式納入，取代第 1 版的 `cpus` 表。2026-09-24 one-boot：沒有 kernel 池了（`kernel` 變保留名）、頂層 `daemon` 也是開 tick 的 daemon、加 `tick_timeout_ms`。）家的長相見 [§1](home.md)；池怎麼增減見 [§3.1](pools.md)。
...
| `daemon` | 絕對路徑 | 見下 | 預設的 daemon 家，**也是替 kernel 開 tick 的 daemon**（one-boot）。所以就算每個池都自己寫了 `daemon`，這格也要寫（舊 info 的 `pools.kernel.daemon` 有寫就先用它） |
...
| `park_ms` | 非負整數 | 300000 | （09-24 停車）行程 `park_ms` 的預設：反覆行程退 102（停車）後最晚多久再派一次，保底用（[§4](echo.md)）。`init` 不寫進 info |
| `bad_after` | 非負整數 | 10 | 連續失敗幾次退件；0＝關掉 |
| `sweep` | 正整數 | 32 | 每格巡檢幾顆忙的 cpu（§3 第 6 步） |

- **`kernel` 是保留名**（2026-09-24 one-boot 起沒有 kernel 池）：`init --config` 寫了 `pools.kernel`＝`FieldTypeMismatch` 拒絕；`add` 的 `pool: "kernel"`＝`-32602`；`cpu add／rm --pool kernel`＝用法錯。
  舊版留下的 info 還有 `kernel` 池：照樣讀得過，但整格略過（不建家、不宣告；`check` 印 warn 叫你刪），它若自己寫了 `daemon` 就當開 tick 的 daemon。
...
| `daemon` | 每個池、還有開 tick 的那個，都要解得出一個 daemon 家；init 時可以還沒有，**boot 才檢查**（`NoDaemon`） | boot 退 1 |
| `tick_timeout_ms` | 非負整數，bool 不算 | `FieldTypeMismatch` |
