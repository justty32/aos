...
沒翻案就照這樣實作。1～12 是第 1 版的（被池式取代的標了），13 起是 2026-09-24 proto5-2 池式納入時選的；跟 kernel 兩邊之間的在 [kernel §10](../kernel/choices.md)。
...
12. ~~改綁 daemon 不支援交接；daemon 重啟後 kernel 要重 boot~~ → kernel 改池的 `daemon` 就走搬池流程；daemon 重啟後鏈多半接得上。
