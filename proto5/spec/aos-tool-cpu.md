...
認領後時，採 60000 ms 預設期限加 30 秒，讓它仍能收屍；正常壞 payload 立即回 BadPayload。
...
| 101 | 沒有可做的工作 |
| 1 | info／I/O 錯、隔離壞共用欄位或結果發布失敗；共用佇列診斷用 aos-cpu 前綴，其餘 `aos-tool-cpu: <代號>: <白話>` |
| 2 | 未知參數或 dir 不是資料夾 |
...
`tick(dir, env=None)` 回 0／1／101，無法繼續的讀驗錯丟 AgentError；`main(argv=None)` 是 CLI 入口。
