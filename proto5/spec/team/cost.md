...
1. **郵差不處理新的 `handoff`（開單）與 `spawn`（生新成員）申請，退件**：照一般退件走（post.md）——原檔進 `rejected/`、紀錄 `code: OverBudget`、`FAILED` 退給寄件人，信文開頭「退件（OverBudget）：財務擋單：超支（<哪一條超了>）」。寄件人是 human（公司總機開的單、人用 `aos-team ask`／`task new` 開的）＝FAILED 進 human 收件匣、`reply_to`＝那份申請的 id，公司總機照它配回總機單、結成 `failed`、轉回下單的部門（company.md），總裁照 SOP 回董事。預算調高（或換週）後**不會自己走**：要做就重新開單。
   - 09-25 五家真跑 §7 第 5 條改的：以前是「原檔留在 outbox、不退件，調高後下一輪自己走」，結果發單的人（總裁、領隊）永遠收不到回音，董事的單永遠不結案。
