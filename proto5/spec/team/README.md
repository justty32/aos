...
| [hr.md](hr.md) | （HR 部 09-25）`aos-team hr`：薪資表 `K/hr/salary.json`（位子→等級→模型→最低通過→證據）、換模型試用 `hr trial`（複製團隊、跑任務集、可插評分指令、記 `trials.jsonl`）、調薪規則、名額（新創：正式員工 10 人、cpu 20／llm cpu 5；擴張後 100／200／25）、擴編規則、正式員工／臨時工（`employment`） |
