...
id＝`<那一次的 epoch 秒×10⁹＋第幾次>-<名字的 crc32>-human`，重跑算出來一樣、不覆蓋；`goal` 前面加 `〔例行 名字 @ 09-25 10:00〕`，`workflow` 沒寫＝`無`。
郵差開單、派給 `to`；單子完成時照任務單的規則通知開單人（human）。

**報告**（漏跑、失敗）：寫進 `team/post/outbox/`，一封給每個領隊、一封給人，`from: post`、`PROGRESS`；id 由事件算出來，重跑不重寄；郵差投。
