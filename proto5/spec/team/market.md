...
- **成功結案**（真跑 09-25 修；五家真跑後再修）：董事的一張單（`company.py order`）算**成功**＝總裁寄了結案信（前台部門寄給 human、DONE、第一行不是〔給 …〕），而且**品管過了**：
  ...
  - **審查輪數**（第 75 題）：每張成功的單配一張有單號的製造總機單（**一張製造總機單只配一張董事的單**：董事的單照下單時間排，每張先拿它下單到結案之間最早一張還沒配走的，再收下一張董事的單下之前還沒配走的＝總裁重發的，用最後一張；沒重疊時＝那段時間最後一張。astra 審查必修 1：以前兩張單時間重疊會配到同一張），看製造部那張單子的 `review`（郵差開的審查子單 `t-xxxx.r1`、`.r2`…的結果，一次一筆 pass）第幾次才過；單子 `failed`＝`FAILED`。`score` 記 `review_rounds`（照成功的順序）與 `review_factor`（照 `review_factors` 換算再平均）。找不到單子或沒有審查紀錄（舊資料、手給的成功）＝當一次過（1.0），`score` 的說明與 `rank` 那列都寫「沒有審查紀錄」。
