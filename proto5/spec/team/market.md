...
2026-09-25 第 1 版（組織設計總監）；同日兩家真跑一輪後修公式（研發部：失敗的單不算分、沒分數不准撥、快＝董事等的時間、省只比成功的、證據代號先展開）。程式：[`lib/aos_market.py`](../../lib/aos_market.py)；指令包裝 [`examples/company/market.py`](../../examples/company/market.py)。
...
- **快**＝**董事等了多久**（真跑 09-25 修；以前是部門間總機單的平均，c1 董事等 401 秒卻記 182 秒）：這一輪（上一輪 `grant` 之後，看結案信的時間，帶時區比）**成功**的董事單，從董事下單（前台部門 human 寄件格裡那封 REQUEST）到總裁結案信的秒數取平均；成功的幾家裡最快的 100，其他＝最快 ÷ 自己 ×100。跨輪完成的單算在結案那一輪。跳數（那段時間裡的總機單數＋1）只記、不算分。
