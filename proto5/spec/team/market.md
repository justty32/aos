...
- **成功結案**（真跑 09-25 修）：董事的一張單（`company.py order`）算**成功**＝總裁寄了結案信（前台部門寄給 human、DONE、第一行不是〔給 …〕）、信裡寫了品管的「結論：合格」、且這段時間裡有結案（done）的品管總機單。FAILED、品管判不合格、沒經過品管的結案都算失敗；還沒結案的不算。`score` 記 `done`（成功張數）、`failed`（失敗張數）；經理人可 `--done N` 覆寫，手給 `--seconds` 沒給 `--done`＝算一張成功。
