...
2. 讀驗（`aos_team_format.read_outbox_file`）；不合＝**退件**：寫紀錄（`kind: rejected`、`code`、`message`）、原檔搬進 `outbox/<名>/rejected/`、寄一封 `FAILED` 給那一格的主人（不是成員也不是 human 的格子＝寄給人），內文是代號＋白話＋原檔搬到哪。
3. 叫 `on_letter`（回報、恢復）→ **投**：收件人是成員＝`members/<名>/input/mail-<id>.json`（`aos_agent_say.drop_new`，不覆蓋）；是 human＝`team/human/<id>.json`（信＋`header` 那一行）。
   投之前先查「是不是已經投過了」：`input/mail-<id>.json` 還在、`input/done/mail-<id>.json.*.done` 在、或收件人 `state.json` 的 `intake.files`／`consuming` 有它——有一個就不投。
   → 叫 `letter_delivered` → 先放 `open/` 標記、再寫紀錄（含全部後續動作，每件 `done: false`）。
4. 原檔搬進 `outbox/<名>/done/`。
5. 逐件做後續動作，**做一件勾一件**（勾之前崩了＝重做同一件；每一件都冪等）。
6. 投給成員的信：之後每輪看 `input/mail-<id>.json` 還在不在，不在了＝收件人收走了，記 `picked_up_at`、叫 `letter_picked_up`。
...
**`verify`**＝建一份驗收工作 `team/post/jobs/v-<單號>-r<rev>-a<attempt>/`（同一個 rev／attempt 只建一次），**不在郵差裡跑**：

- 有 `AOS_KERNEL_HOME`（kernel 叫的郵差都有）＝`aos-kernel add --once` 一份 inst（`aos-team verify 單號 --rev R --attempt A --out <工作>/result.json`）；
- 沒有（人手動跑 `aos-team post`）＝另開一個行程跑同一行，不等它。

之後每輪看 `result.json`：出現＝`verified` 事件；kernel 的回音到了卻沒結果檔、行程死了、或超過 10 分鐘＝這次壞了，重交（最多 3 次），還是不行＝寄 `BLOCKED` 給人、單子停在 verifying。
kernel 的回音讀完會 ack；做完的工作搬進 `team/post/jobs-done/`。
...
{"_metainfo": {"_type": "aos_team_post_record", "_version": 1},
 ...
 "src": "…/team/outbox/worker-1/1790000000123456789-4242-worker-1.json",
...
`kind`：`letter`（信，含郵差生的）、`request`（申請；多 `request_kind`）、`rejected`（多 `code`、`message`）、`notice`（郵差自己起的：停滯 `stall.<單號>.<雜湊>`、期限 `expire.<單號>.r<rev>`）。
...
- **健康不是 ok**（`aos_agent_status.collect` 的 health：retrying、paused、bad、kernel 壞、沒登記…）持續 60 秒以上；
- 否則**超過 `limits.stale_minutes` 沒進展**：最後進展＝單子最後一次變動、成員記憶檔最後一次寫、事件紀錄（`log/events.jsonl`、`team/events/<名>.jsonl`）最後一次寫，取最晚。
...
期限：過了 `deadline` 還沒結束的單＝`expire` 事件（開一份 `notice` 紀錄做這一步，崩了照樣做完）。

## 書記

專案裡有 `SESSION-LOG.md`（或 `wf/SESSION-LOG.md`）才寫，沒有不建：`## 最新進度` 那一節換成「人寫的行＋每張還沒結束的單一行」：

```text
- [t-0001 IMPORT.md] 做事中（worker-1，第 1/3 次）→ 等 worker-1 回報：把 workflows 的 heartbeat 包導入…
```

`WAIT_USER.md` 的 `## 待使用者項`：每個 open 的問題一行 `- [q-0001] worker-1 問：…（選項：…） → aos-team answer q-0001 "…"`。
書記的行認法：`- [t-NNNN` 或 `- [q-NNNN` 開頭；其他行原樣留著；一行都沒有＝`（目前無）`。標題不在就加在檔尾。內容一樣不重寫。
...
| `aos-team start`／`stop`（第 1 隊） | 會叫 `aos_team_post.start(團隊資料夾)`：登記 kernel 反覆工作 `team-post-<資料夾名>`，inst 在 `team/post/post.inst.json`、stderr 在 `team/post/post.err` |
