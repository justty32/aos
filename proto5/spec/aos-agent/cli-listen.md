...
- **`--last [N]`**（就是舊的 `last`）：N 省略＝1；給了要是正整數（只收 `0-9` 組成、≥1，前導 0 可），不是＝用法錯 2（`--last 後面要是正整數…`）；沒有上限，超過十億位數也只是「全部」。讀驗 info 後找記憶裡**最後 N 則回話**、照時間順序印出來、退 0。
...
**時間**：記憶本身沒有時間戳，這裡是「收下那輪輸入的時刻」，取自輸入封存檔名 `<原檔名>.<ns>-<pid>.done`（[agent §4.1](../agent/state.md)）裡的 ns：`input` 每個路徑是資料夾就看 `<它>/done/`、是檔就看同層 `done/` 裡 `<檔名>.*.done`；同一個消費 id（`<ns>-<pid>` 整串）＝同一次收件，裡面照 `input` 路徑順序、再照**原檔名**排（跟收的時候一樣）。只讀普通檔（symlink、FIFO 等略過）；ns 大到換不成時間的那筆不印時間。
每輪第一句 user 的 `content` 跟收件（照時間排）裡第一句 user 比對，往後找、不回頭；對不上（封存被清掉、手改記憶、`state.json` 讀不了）就只印 `── 第 R 輪 ──`。
這是**推定**：兩輪說了一模一樣的話、又只清掉前一輪的封存，時間會配錯一輪；截掉記憶前半、改過 `input` 路徑也可能對錯。要精確時間得讓記憶自己記，這版沒做。

### `--show-calls` 與 `--show-calls-full`（09-24 listen 微調）

兩個互斥（都給＝用法錯 2），對三種看法都有效。開了之後印的是**一段記憶**而不只是回話：user 不印，叫工具的 assistant 印它的字（有的話）再印呼叫行，`role: tool` 印結果行（工具名用**它之前最近**一個同 `id` 的呼叫對回來——不同輪可以重用 `id`；對不到寫 `?`）。

- 哪一段：`--last N` 是「倒數第 N＋1 則回話之後」到記憶尾（所以第一則回話那輪叫的工具也看得到；但同一輪更早、掛在另一則有字回話上的呼叫不在這段裡——要看就把 N 加大）；`--wait` 是 H0 以後這一輪；`--follow` 是每多出來的一格即時印（叫工具那一刻就印呼叫行，不等結果）。
- **`--show-calls`**（簡化）：呼叫一行 `[呼叫 名 k=v k=v]`——參數解成 JSON 物件就逐個 `k=v`（字串原樣、換行寫成 `\n`、別的型別印 JSON），每個值超過 40 字截成 40 字加 `…`，整段參數超過 120 字再截；工具名、鍵、值裡的 LF／CR 一律寫成 `\n`／`\r`，保證一個呼叫就一行；沒參數＝`[呼叫 名]`；參數不是 JSON＝`[呼叫 名 參數不是 JSON：<前 40 字>]`。
...
- 記憶形狀怪（`info.json` 壞了改讀、沒驗過的 `prompts/history.json`）：不是物件的格略過；`tool_calls` 不是陣列當沒有、裡面不是物件或沒有名字的呼叫寫 `?`；`arguments`、`content` 不是字串就印成 JSON。一律不當掉。
