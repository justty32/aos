...
兩支都不叫模型、只用標準庫。錯誤印一行 `指令: 代號: 白話` 到 stderr、退 1；用法錯先印原因再印用法、退 2；`-h` 印用法退 0。
...
- **版本**：每次寫之前把舊的整份存到人格檔旁邊的 `.versions/<檔名>-<奈秒>.json`，留最近 20 份；`versions` 列、`revert [ID]`（省略＝最近一份）還原——還原前也先存一份，所以還原可以再還原。內容沒變＝印「沒改」、不寫、不存版本。
...
- `aos-json` 不做鎖：人跟 agent 同時改同一個檔時靠 `--expect-sha`。
