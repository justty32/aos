...
> 2026-09-24 one-boot：K 帳本換成 `K/ledger.sqlite`，aos-agent 不再直接讀 `K/state.json`，改用跟 `aos-kernel proc` 同一支 lib（`lib/aos_kernel_store.py`）只查需要的那一列——§11 start 的相容檢查（舊 `state.json`＝`KernelIncompatible`、`features` 改讀 sqlite）與已登記判定、§5.2 查有沒有放過、§10 清工作檔跟著改；§2.1 kernel 同時一格的理由、§1.7 check 的 PATH、`talk` 的 health 清單、常見狀況表的「沒在跑」改寫。
