...
快照是 `error`、找不到 `bwrap`、或 `_meta` 用 `$env` 讀了敏感名字（`EnvUnsafe`）＝這件記成沒執行、不送；`done.content` 是寫給模型看的一段話（講被 aos 擋下、叫它轉告使用者去跑 `aos-agent check`），不帶細節，格式在 [access.md §2](access.md)。牢裡不繼承 cpu 的環境（上一段的「整包繼承」只對不關牢的工具成立）。細節與例子在 [access.md §2](access.md)。
