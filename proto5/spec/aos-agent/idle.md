...
1. `intake` 是 null：列出 `input` 指到、現在存在的檔（[agent.md §4.1](../agent/state.md)）。一個都沒有＝退 101。
...
3. 從每個 `dst` 讀訊息、照 agent.md §3.2 驗。壞檔＝退 1，**已寫的 `intake` 與已搬的檔都留著**。人要處理：就地改好那個 `dst`；
   或照 [agent.md §4.4](../agent/state.md) 放棄那一對（先 stop、在 state 裡拿掉那一對、才動 `dst`）——只刪 `dst` 不改 state，下次會去搬原路徑上的新檔。全部略過＝沒輸入：寫 `intake: null`、退 101。
