...
5. **寫 state**：`state: think`、`intake: null`。退 **103**（09-24 tick-gap：馬上能問模型；以前退 0 白等 `interval_ms`）。（09-24 第 4 隊補）寫 state 之前記一筆事件 `intake`（[agent/events.md](../agent/events.md)）。
