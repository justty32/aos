...
| 碼 | 什麼時候 |
|---|---|
| 0 | 這格做了事：收到一部分回音而全到了只差 ack、結清但問模型失敗（含連敗暫停，`interval_ms` 當退避）、自動壓縮、收輸入時輸入檔都不見了；崩潰恢復時重送了上一格送過的批；（09-24 fix-r4 補）手動暫停中（什麼都沒做） |
| 103 | （09-24 tick-gap）**做了事、下一步馬上能做**：結清成功（接著要送工具、再問模型或看輸入）、收完輸入（接著問模型）、`act` 卻沒有 tool_calls（改回 `think`）、整批都在本地結束（沒有要等的，接著結清）。kernel 馬上再排（[kernel §4](../kernel/echo.md)），不等 `interval_ms`；舊 kernel 不認得，所以 `start` 要帳本 `features` 有 `again` |
| 101 | 在等：門沒開；（09-24 fix-r4 補）拿不到 tick 鎖（另一個 tick 正在跑，§2.1） |
| 102 | （09-24 停車）**停車**：批在途、這格什麼都沒收到、什麼都沒寫（§6 第 5 步）；（09-24 tick-gap）剛送出一批（每件都帶 `wake`，回音出貨時 kernel 叫醒它；回音比它退出還早到也不怕，kernel 記 `woken`、馬上再排）；收到一部分、其餘還在途；`idle` 沒輸入（§8 第 1 步：沒有 `intake`、也列不到輸入檔，自動壓縮也沒做事）。kernel 等到它等的回音出貨、或有人投 `wake`（say／talk／郵差投完輸入就投）才再派，最晚 `park_ms`（預設 5 分鐘，[kernel §4](../kernel/echo.md)）。門關著照舊 101：門是外人開的，kernel 叫不到 |
