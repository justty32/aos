...
| **連敗暫停**：問模型連續失敗 3 次 | `status` 第一行 `連敗暫停`、`wait` 行寫「連敗暫停」；`log/agent.err` 有 `stuck` 行；`aos-kernel ls` 那行標 `連敗暫停中` | 修好原因（endpoint、模型代號、逾時），`aos-agent continue`（好幾個一起就 `continue --all`） |
