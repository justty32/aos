...
十份規範，一份一個資料夾（2026-09-24 proto5-2 池式納入：kernel、daemon、cpu 三份改成池式，沿革見各資料夾的 history.md）；每個資料夾的 `README.md` 是入口（一句定位、已拍板的前提、各節一覽），內容依原本的 § 節拆成小檔，節號照舊，文中「§3」「§1.1」這類引用去該資料夾的 README 查在哪個檔。
...
| [daemon](daemon/README.md) | daemon：所有 cpu 的父行程，按池照宣告啟動、重拉、停止 |
| [kernel](kernel/README.md) | kernel：排程也是一格一格的 aos-exec，替登記的工作挑空 cpu、收結果、決定要不要再跑；cpu 按池管（`cpu add／rm／ls`） |
