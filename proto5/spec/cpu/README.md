...
cpu 的環境就是工作的環境（llm cpu＝環境裡有 `llm-http` 的普通 exec cpu）；cpu 死了由 daemon 重拉（2026-09-24 池式納入後照池的宣告，[daemon §4](../daemon/loop.md)）。
...
誰拉 cpu 起來、怎麼知道它死了、重拉、停機階梯（[daemon](../daemon/README.md)）；誰決定哪個目標去哪顆、
...
| [notify.md](notify.md) | §6.4 回完音丟一張通知（`notify` 欄位，2026-09-24 池式納入加） |
