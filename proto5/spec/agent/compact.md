...
所以 `history.json` 從來不缺、不會是半截。archive 預設全留；`compact --prune-archive 天數` 刪超過天數、而且現在的記憶沒提到檔名的。
...
縮不動（還是超過、或 `HistoryInvalid`）就把「記憶 sha＋選項」寫進 `log/compact-skip`，同一份記憶不再每格重算；記憶一變就又會看。錯誤進 `log/agent.err`（`aos-agent: compact: …`）與事件 `compact_fail`，不擋收輸入。`info.compact` 寫壞了不自動縮，也不每格噴錯（`check` 另查）。
...
處理函式把申請投成那個成員家的 `compact-req/<申請 id>.json`（暫存檔＋link，不覆蓋；`compact-req/done/` 已有同 id 的也不再投），回空的後續動作清單。
tick 看到 `compact-req/*.json` 就縮（多份一起算一次，選項以檔名排序最後一份有寫的為準），然後把它們搬到 `compact-req/done/<檔名>.<消費 id>.done`；**先縮再搬**，崩在中間重跑是空轉＋搬。
