...
| `ev` | 事件種類（下表） |
| `id` | 去重用的身分：批 id（工作名去掉最後的 `-<序號>`，例 `aw-bob-1790000000000000000-77`）、消費 id、或壓縮前記憶的 sha；沒有＝`null` |
...
- **至少一次**：每個事件都在「提交那一步」之前寫；崩了重做會再寫一次同一行（同 `ev`＋`id`）。讀的人照 `ev`＋`id` 去重（`id` 是 `null` 的不去重），`aos-agent events` 已經去重。所以只會「記了兩次」，不會「做了沒記」。
