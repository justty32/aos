...
- 每段問一次模型（temperature 0、`max_tokens` 1024、不帶工具），送的是整段中間（讓它看得懂上下文），系統提示要它：不重述使用者原話、行數與數字用阿拉伯數字照寫、工具呼叫的檔名照寫、不編造、只回本文、越短越好。回的前後 ``` 圍欄會去掉；圍欄外只有一兩句客套話（各 ≤ 1 行、合計 ≤ 80 字）＝只取圍欄裡的，開了沒收尾＝去掉第一行；其他不猜，交給下面的檢查（09-25 收尾 S4）。
