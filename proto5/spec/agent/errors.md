...
（09-24 access-impl）權限牆（[§3.5](access.md)）：`access.json` 格式錯、路徑不在、指示詞解不過＝`AccessInvalid`（本身不是 JSON＝`JsonSyntax`，帶行列）；可寫 mount 碰到信任資料＝`AccessUnsafe`；要關牢卻找不到 bwrap＝`NoBwrap`。這三個只讓當批的工具「跑不起來」，不擋 tick。
