...
| # | 篇 | 一句話 |
|---|---|---|
| 01 | [設定、啟動與關閉 daemon 和 kernel](01-daemon-kernel.md) | 環境檔、`llm.json`、`kernel.json`（池表：每池幾顆）一次寫好；開機、檢查、關機、每天重開機 |
...
| 06 | [附錄：不用 init，手寫一個 agent 家（和 kernel 家）](06-appendix-manual-home.md) | agent 家就是一個資料夾加三份檔；kernel 家就是一份池表（`K/info.json`） |
