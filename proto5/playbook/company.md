...
一段＝一季：董事 `order` → 總裁派〔給 mfg〕→ 製造 → 總裁派〔給 qa〕→ 品管 → 總裁回報董事；季末經理人 `market.py score／rank`（品質×成功率×審查係數、快只比成功張數相同的、省；09-25 晚第 74、75 題）→ 花光的先倒閉 → `grant`、剩兩家合併；每隊收尾沉澱進 playbook（施工層）與 commons（運轉層）。
