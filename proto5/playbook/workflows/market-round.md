...
- **打分前看一眼品質怎麼來的**（兩家真跑 09-25）：`--eval` 吃的是機械檢查，證據檔換了寫法（例如「A 檔 L30」代號）就整片讀不到、做好的判成 50；先看 evidence_check 的 `by_status`。單子 FAILED 的公司品質照樣可能 100，這輪用 `grant --tokens 名=0` 覆寫（lessons 29、30）。
- **發錢前確認這輪有人打過分**：`grant` 不管有沒有新分數都照排名發，重跑一次就多發一輪；先 `rank` 看品質、結單不是全 0（lessons 31）。
