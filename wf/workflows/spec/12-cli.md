...
- **S-12-02** 退出碼照表：`0` 正常、`2` 用法錯、`3` 解析或版本拒絕、`4` 不是地、`5` run 因失敗或預算停、`75` 沒跑成、`130` 被取消；禁止據此判斷可否重試。〔主編補〕
...
## 請求、原稿與收件匣
- **S-12-51** `aos status --request <id>` 必須讀 LLM 世界的 `.aos/requests/<id>.json`，印出那筆請求是 `queued`、`sent`、`done` 還是 `failed`。〔裁決 2026-09-05〕
...
- **S-12-57** `--until` 必須收 `idle` 與 `never` 兩個值；`never` 是閒著也不停、只等投遞或控制信。〔裁決 2026-09-05〕
...
- S-12-51 `--request <id>` 查 LLM 請求狀態。〔裁決 2026-09-05〕｜S-12-52～54 原稿雜湊、重開才生效、兩收件匣分開去重。〔主編補〕
- S-12-57 `--until` 收 `idle`／`never`。〔裁決 2026-09-05〕｜S-12-58 `aos daemon add` 只登記不開跑。〔主編補〕
