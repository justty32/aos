...
- **S-12-22** `aos mv <來源地> <目的地>` 必須是停時鐘＋搬＋重新登記三步，並且文件必須把它寫成唯一正確的搬法。〔預設 2026-09-05，C-04〕
...
- **S-12-56** `aos llm serve <地>` 必須是 LLM 世界專用的迴圈：預設 `--every 200 --until never`，跟 exec 搶同一把 `.aos/lock`，退出碼同 `aos run`；daemon 對 LLM 世界必須起它，禁止起 `aos run`。〔主編補〕
...
- S-12-22 `aos mv` 要做，並寫成唯一正確搬法。〔預設，C-04〕
...
- S-12-56～58 `aos llm serve`、`--until never`、`aos daemon add` 只登記不開跑。〔主編補〕
