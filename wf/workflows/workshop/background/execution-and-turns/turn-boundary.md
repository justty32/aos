...
**在 aos 裡具體是什麼**：三支都已存在，實作在 `core/inst/src/handoff.cpp`，都是以 instruction 檔路徑為參數的公開函式，由 `core/inst/src/run_exec.cpp` 依序呼叫；規格見 [aos-folder](../../../../../docs/aos-folder.md#六交接協定三步每步一次-rename)。把它們另外長出 argv 入口是**提案**，目前不存在。
...
**在 aos 裡具體是什麼**：**已存在，而且是刻意的**——見 [aos-folder](../../../../../docs/aos-folder.md) 第十二節〈已經被實作決定的〉。`timeout_ms` 那條路徑已經有對整個 process group 送 `SIGTERM`→`SIGKILL` 的處理（`core/inst/docs/exec.md`〈逾時與行程群組〉），缺的只有「parent 被外部信號砍掉」那一條路徑。
