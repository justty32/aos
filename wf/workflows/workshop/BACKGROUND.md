...
這份不是第八份結論，而是查表。先查名詞，再跳到要回答的題目；每個「目前」都以 [`.aos` 資料夾規格](../../../docs/aos-folder.md) 與 [roadmap](../roadmap.md) 為準，研討會產生但尚未轉交進規格的東西一律標成「提案」。
...
- no-replace／`renameat2(RENAME_NOREPLACE)` → [投遞契約與追蹤](background/delivery-contract.md)
- consumer acknowledgment → [投遞契約與追蹤](background/delivery-contract.md)
- TOCTOU → [投遞契約與追蹤](background/delivery-contract.md)
