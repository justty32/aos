...
| `test_run_init.cpp` | init、額外 argv 拒絕、`turn` 初值 `0`，以及 init／exec 的目前目錄預設 |
| `test_run_loop.cpp` | loop argv、連續回合、失敗節流、信號收尾、遇 3 停止與目前目錄預設 |
| `test_run_handoff.cpp` | CLI 的版本、空回合、彙整、隔離、取件、釋放與連續回合整合；`.aos/turn` 在有工作的回合遞增、空轉不動、缺檔的舊世界視為 `0`（裁-5） |
