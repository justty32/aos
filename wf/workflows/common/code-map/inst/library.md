...
| `handoff_aggregate.cpp` | **handoff 層的彙整動作**（從 `handoff.cpp` 拆出，單檔行數）：掃收件匣→攤平成一批→排他發布成 base→維護 header sidecar。發布順序照 SPEC §D-5：唯一名寫批→rename 進固定 `.temp` 槽位→唯一名寫 header→**rename header（去重承諾的提交點，`swept` 寫 `false`）**→`fsync_dir`→**排他** rename 批（`publish_exclusive`；`EEXIST`＝別人先發布了，本輪放棄、不清投遞、不標 swept）→`fsync_dir`→刪投遞→`fsync_dir`→全數成功才把 header 改寫成 `swept:true`。去重（§D-6）的閘門是 `swept`：只有現任 header 的 `swept` 不成立才啟用 id 比對，roll-forward 另要求固定槽位的**位元組逐位元等於本輪重算的 canonical 位元組**。投遞出問題分三條路：內容無效→隔離成 `.bad`（排他，撞名換唯一名，絕不覆蓋）、讀不到→留在原地記 `DeliveryReadFailed`（`ENOENT` 靜默）、非普通檔→跳過記 `DeliveryNotRegular`（擋 FIFO 卡死 `open`）；以 `.json` 結尾但形狀不合的名字記 `DeliveryNameIgnored` 出聲。`opendir` 用 RAII guard 持有。**同一輪 `HeaderWriteFailed` ＋ `DeliveryRemoveFailed` 一起出現＝致命**（回 `PublishWriteFailed`，不新增列舉值），否則是無上限的副作用重播 |
