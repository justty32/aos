...
handoff 層有四個 `.cpp`：`handoff.cpp`（取件／釋放）、`handoff_aggregate.cpp`（彙整）、`handoff_deliver.cpp`（投遞），加上兩組內部標頭 `handoff_fs`／`handoff_header`。
...
| `handoff_aggregate.cpp` | **handoff 層的彙整動作**（從 `handoff.cpp` 拆出，單檔行數）：掃收件匣→攤平成一批→排他發布成 base→維護 header sidecar。發布順序照 SPEC §D-5：唯一名寫批（`unique_temp_path`）→唯一名寫 header→**rename header（去重承諾的提交點，`swept` 寫 `false`）**→`fsync_dir`→**排他** rename 批（`publish_exclusive`，來源就是自己那份唯一暫存；`EEXIST`＝別人先發布了，本輪放棄、`unlink` 自己的暫存、不清投遞、不標 swept）→`fsync_dir`→刪投遞→`fsync_dir`→全數成功才把 header 改寫成 `swept:true`。**沒有共用的 `<base>.temp` 固定槽位**——共用可變狀態會讓兩個彙整者互相覆蓋對方的批，於是 A 可能發布 B 的批卻只清 A 看到的投遞，剩下的下一輪重跑＝重複執行。去重（§D-6）的閘門是 `swept`：只有現任 header 的 `swept` 不成立才啟用 id 比對；roll-forward 的錨靠 `find_rollforward_anchor` 掃兄弟唯一暫存（`<stem>-*.json.temp`，跳過 `<stem>-head-*`，排序後逐一讀），**位元組逐位元等於本輪重算的 canonical 位元組**的第一個才算——身分由內容決定，名字只是索引；撞 `EEXIST` 時不刪錨（那不見得是自己的檔）。崩在「寫完批、還沒發布」之間會留下 `<stem>-<pid>-<seq>.json.temp` 殘骸，**刻意不掃刪**（§D-8，清理歸人或 M3 的 `aos recover`）。投遞出問題分三條路：內容無效→隔離成 `.bad`（排他，撞名換唯一名，絕不覆蓋）、讀不到→留在原地記 `DeliveryReadFailed`（`ENOENT` 靜默）、非普通檔→跳過記 `DeliveryNotRegular`（擋 FIFO 卡死 `open`）；以 `.json` 結尾但形狀不合的名字記 `DeliveryNameIgnored` 出聲。`opendir` 用 RAII guard 持有。**同一輪 `HeaderWriteFailed` ＋ `DeliveryRemoveFailed` 一起出現＝致命**（回 `PublishWriteFailed`，不新增列舉值），否則是無上限的副作用重播 |
...
| `handoff_fs.hpp`／`.cpp` | **內部標頭**（不對外）。handoff 層的路徑推導與低階檔案存取：從 base path 推出 `.runi`／`.tempd`（`HandoffPaths`，**沒有 `.temp` 欄**：彙整不再產生固定槽位）、EINTR-safe 的整檔讀寫（`write_file` 在 close 前 `fsync`；`write_file_exclusive` 同款但用 `O_EXCL` 建檔；`error` 保留**先發生**的 errno，`close` 只在前面沒出錯時才有資格寫）、`fsync_dir`、`publish_exclusive`（排他發布：優先 `renameat2(RENAME_NOREPLACE)`，不支援時退階 `link`＋`unlink`，目的檔已存在一律回 `EEXIST`、不覆蓋；**退階路徑收尾的 `unlink` 失敗回 `true`**，errno 走可選出參 `leftover_error`——目的檔已經連好，謊報失敗會害生產者重投）、`next_unique_token`／`next_delivery_name`（`<pid>-<seq>`，行程內 atomic 計數）、`unique_temp_path`／`unique_bad_path`（把權杖插在最後一個 `.json` **之前**再接狀況字，仍符合 §B-1 文法：`inst.json`→`inst-4711-0.json.temp`）、`parent_directory`、投遞檔名判定與 path join。不碰 `HandoffResult`、不做交接決策；`.cpp` 這個檔需要 `_GNU_SOURCE`（`renameat2` 是 GNU 擴充，宣告在 `<cstdio>`），其他 handoff 檔仍用 `_POSIX_C_SOURCE 200809L` |
| `exec.cpp` | **exec 層**：唯一碰 `fork`／`execve`／`waitpid` 的檔。`execute()`：組 `argv`＋`envp`（透過 `spawn_prep`）→ `fork` → 子行程 `setpgid`＋重導向（stderr merge 時在 stdout 設好後 `dup2(1, 2)`）＋`chdir`＋`execve`（`run_child`，全程 async-signal-safe）→ 父行程視 `timeout_ms` 決定直接 `wait_retry` 或 `wait_until` 輪詢；逾時先對整個行程群組送 `SIGTERM`、給 `kTimeoutGraceMs`（2000ms）緩衝，仍不收就 `SIGKILL` 整個群組——打群組是因為忽略 `SIGTERM` 的孫行程才殺得掉。結束後若 `exit_path` 非空就把 exit code 寫進那個檔（`O_CLOEXEC` 開檔——這是唯一活在「會 fork 的多執行緒行程」裡的 open；close 前 `fsync` 檔案、close 後 `fsync` 父目錄，因為新建檔的目錄項也要落盤，這是崩潰後對帳的證據）；重導向用的 `child_redirect` 只開檔（`O_CLOEXEC`，`dup2` 出來的目標 fd 不帶 CLOEXEC 所以重導向照活；`open` 剛好拿到目標 fd 時改用 `fcntl(F_SETFD, 0)` 清掉）＋`dup2`，內容是子行程寫的，不 fsync（已知豁免）；**開檔失敗會用 `write(2)` 往 fd 2 噴一行 warning**（fork 之後只能 async-signal-safe，所以不 `fprintf`、不串接字串、連 `strlen` 都自己數），退出碼仍是 126 |
