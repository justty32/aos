...
| `capi_io.cpp` | 讀寫整份 instruction：`read_buffer/fd/file`、`write_buffer/fd/file`（呼叫 `format` 層），以及 `aos_instruction_execute`（呼叫 `exec` 層）。`write_file` close 前 `fsync`（S7 fsync 掃尾）；`write_fd` 寫的是呼叫者自己的 fd，**不**代呼叫者 fsync（已知豁免，呼叫者自行決定落盤時機） |
