...
| `capi_io.cpp` | 讀寫整份 instruction：`read_buffer/fd/file`、`write_buffer/fd/file`（呼叫 `format` 層），以及 `aos_instruction_execute`（呼叫 `exec` 層）。`write_file` close 前 `fsync`，close 之後再 `fsync` 一次父目錄（新建檔的目錄項也要落盤——`AOS_INST_OK` 是對外承諾的一部分）；`write_fd` 寫的是呼叫者自己的 fd，**不**代呼叫者 fsync（已知豁免，呼叫者自行決定落盤時機） |
