...
| `run_init.cpp` | 建立 `.aos/`、版本 1、`inst.tempd/` inbox 與回合計數器 `turn`（初值 `0`，§B-3），失敗時回滾剛建立的狀態。`version`／`turn` 兩個檔各自 close 前 `fsync`，寫完後再 `fsync` 一次 `.aos` 目錄 fd 把三個新目錄項（`version`／`inst.tempd`／`turn`）一併落盤（S7 fsync 掃尾） |
