...
| [proto5/README.md](../../../proto5/README.md) | `proto5/` 規範先行的重寫：`spec/`（directives／inst-posix／exec 三份規範）、`lib/`（`aos_directives.py` 指示詞機制、`aos_inst.py` inst 讀驗、`aos_exec.py` 執行者＋測試）、`bin/aos-exec` 入口 | 要改 proto5 的指示詞機制、inst.json 讀驗、aos-exec 的行為或命令列 |
