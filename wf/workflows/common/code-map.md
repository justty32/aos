...
| [proto5/README.md](../../../proto5/README.md) | `proto5/` Python 3.12 原型；完整模組／API 導航見 [lib/README.md](../../../proto5/lib/README.md)。底層 directives／inst／exec；新 home／client／exec_cpu／daemon／kernel 按 cpu／daemon／kernel 三份規範運作，CLI 為 aos-cpu／aos-daemon／aos-kernel；舊 agent／llm／tool cpu 保留，尚未遷移 | 要改 proto5 的指示詞、inst／exec、JSON-RPC 家與交件、cpu 執行、daemon 孩子管理、kernel 帳本／出貨箱／tick 鏈，或查保留的 agent／llm／tool API |
