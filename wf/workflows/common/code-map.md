...
| [proto5/README.md](../../../proto5/README.md) | `proto5/` Python 3.12 原型；完整模組／API 導航見 [lib/README.md](../../../proto5/lib/README.md)。底層 directives／inst／exec；新 home／client／exec_cpu／daemon／kernel 按 cpu／daemon／kernel 三份規範運作，CLI 為 aos-cpu／aos-daemon（含 stop）／aos-kernel（含 ack、ls 摘要）；agent 線已接上 kernel：aos-llm-call（問模型一次）、aos-agent tick／start／stop（`aos_agent*.py` 六支，崩潰窗口與真 daemon 整合測試在 `lib/test/test_agent_*.py`）；舊 llm／tool cpu 已移除 | 要改 proto5 的指示詞、inst／exec、JSON-RPC 家與交件、cpu 執行、daemon 孩子管理、kernel 帳本／出貨箱／tick 鏈，或 agent 家讀驗、aos-llm-call、aos-agent 的批次／恢復 |
