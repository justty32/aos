...
| [proto5/README.md](../../../proto5/README.md) | `proto5/` 規範先行的重寫：`spec/`（directives／inst-posix／exec／agent／aos-agent／aos-llm-ask 六份規範）、`lib/`（`aos_directives.py` 指示詞機制、`aos_inst.py` inst 讀驗、`aos_exec.py` 執行者、`aos_agent_info.py` agent 資料夾讀驗、`aos_llm_ask.py` 問模型一次＋測試）、`cli/aos-exec`／`cli/aos-llm-ask` 入口 | 要改 proto5 的指示詞機制、inst.json 讀驗、aos-exec 的行為或命令列、agent 資料夾（info.json）怎麼讀驗、aos-llm-ask 怎麼組請求／打引擎 |
