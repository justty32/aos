...
← [dispatch](../README.md)｜隊 A [proto-A-machine](done/proto-A-machine.md)（已收線）｜隊 B [proto-B-agent](done/proto-B-agent.md)（已收線）
...
| `every/` | 任何生產者 | 常駐投遞匣。每回合**複製**（不搬走）成一條指令，id＝`<stem>-<turn>`；可選正整數 `every_ms` 讓該檔僅在距上次投遞達指定毫秒時再投，未設定或值 ≤ 0 則維持每回合投遞，上次投遞的 epoch 毫秒記在 `every/.last/<stem>`。檔案一直留著，直到有人刪掉 |
...
**同一回合內，批中各筆 inst 之間沒有資料流**：整批是並行 fork/exec，沒有先後可言，一筆不能引用同批前一筆的產物（`wire::Inst`＝§2 也沒有 `$ref` 或等價欄位可以這樣做）。有依賴的兩筆要拆到不同回合——後一筆等前一筆的 `out/` 寫完，再由生產者投遞（`aos deliver` 或下一輪的 `every/`）進下一回合的 `inbox/`。這是刻意的語意，不是還沒補的限制。
