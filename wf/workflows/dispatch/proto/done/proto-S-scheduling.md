...
- top-down-cli §三：「LLM 不是被呼叫的函式，是另一台跑同一套協定的機器」——把「想一次」投遞到 llm pu 資料夾、本地回合輪詢結果。
- ideas/llm-cpu.md（若存在）、workshop [finite-resource-queue](../../../workshop/records/finite-resource-queue.md)（五位一致要「使用者層級的 endpoint 佇列」）、使用者 2026-08-25 的話：「外部處理器自己監控一個資料夾、甚至不必引用 aos lib」（排隊是外部處理器的家務）。
- ideas/tools/call-loop.md（三回合往返）、nested-worlds、usability-target（第二級：agent 團隊）。
- 采風 ai-core-field/synthesis.md：ai_core 對 resources／lifecycle 軸的主張。
...
> **使用者具體化（2026-08-30）**：「假如我設定只允許同時有三個正在跑的 DeepSeek API 呼叫」——每顆 CPU 一個使用者設定的並行上限（如 deepseek: 3、lmstudio: 1），超過就排隊。方案要用這個案例走一遍；拍板清單加「上限設定住哪、超上限排隊還是退回」。
