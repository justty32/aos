...
**寫程式的手與跑程式的手讀同一種 form**，這才是同像性在撐的東西。

> **同日下午多一個答案候選（未裁）**：C 語言線把「源碼 → IR → `series` → `inst` 批」定成
> **每一站同一種樹、拆平器只是同型改寫**，所以同像性也撐**編譯管線的簡單**——而「改寫器
> agent 也會寫」與自我改寫是同一件事兩面 →
> [assembly-and-chains/source-and-ir](assembly-and-chains/source-and-ir.md)。Python／Lua「也適合」
