...
| [agent-loop-under-clock](agent-loop-under-clock.md) | **agent 規劃第一篇**（2026-09-04）：agent 是一塊地加逐 tick 小狀態機；LLM／慢工具各生脫節小地，兩個有事 tick 中間夾空等 tick。使用者確認此走法（傾向，非裁決）；一次性時鐘、等待時世界會變與 inotify 門鈴仍保留，**三題已答**，見下一列 |
| [agent-loop-answers](agent-loop-answers.md) | **agent 三題的使用者回答**（2026-09-04）：一個 agent＝一個資料夾；跑到 LLM 不再叫工具、list 空就閒著等下一封信；`aos run` 通用停法＝本次 exec 沒產出新 inst 就停；prompt 組法留給工程。使用者另宣告概念層收尾，但「先玩現有系統」裁決仍有效 |
| [land-rules](land-rules.md) | **[daemon-clocks](daemon-clocks.md) 的續篇**（2026-09-04，**無明說裁決**）：使用者定義一塊地預設只看自己，掛載／symlink 才能開洞；刪資料夾就是死，路徑是唯一標示符，搬家等於死。AI 建議 daemon 同查 pid＋路徑、孤魂補一刀；搬家分析另開 [land-rules-move](land-rules-move.md)，現在不做 `git mv` 式通知，真要搬就停時鐘後當新生 |
| [top-to-bottom](top-to-bottom/README.md) | **「資料夾＝list」整套模型的大白話總整理 01～09**（2026-09-03～09-04，**無新裁決、無新主張**）：空間從 Linux 一路講到原子 inst；08 補時間（tick／時鐘／async／daemon）；09 收束 LLM 當虛假 CPU——主機板已夠，四個工程腳位留給實作。各檔 ≤ 8 KB，三種聲音（使用者原話／裁決／AI 觀察）逐節標明 |
...
| [fuse-host-why](fuse-host-why.md) | **fuse-host 三篇的前情補述**（2026-09-04，**無裁決**）：最初動機其實混了兩題——少碰磁碟與偵測操作。第一題先用 tmpfs（零程式碼，且 FUSE 反而多繞核心）；只記帳再用 inotify，真要擋或演假檔案才用 FUSE。含現有四個 core 的同步寫、每回合寫入量與 `.aos/` 搬 tmpfs 的對照 |
| [fuse-host-extras](fuse-host-extras.md) | **[fuse-host-why](fuse-host-why.md) 的續篇**（2026-09-04，**無裁決**）：門房的額外處理分三級——inotify 只看不擋、FUSE 真擋、FUSE 變出假檔案；總規則是敲門時只記／擋，耗時工作回到 tick。使用者傾向 FUSE 要做但未裁時程，「先玩現有的」仍有效；順序仍是 tmpfs → inotify → FUSE |
