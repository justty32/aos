...
| [nested-eval-sugar](nested-eval-sugar.md) | **[nested-eval-car](nested-eval-car.md) 的續節**（2026-09-03，**含一條裁決**）：使用者觀察「inst 攤平／接力棒／`out/` 在這種情況下變成一種語法糖，因為複雜邏輯可以直接託付給子資料夾」，**同日裁定：inst 鏈是為省成本而存在的語法糖，不是本體**——本體只有**原子 inst**（工具／LLM 呼叫，不是資料夾、無法再往下託付）與**開／讀／選**。AI 觀察：**攤平＝內聯**（開子資料夾＝生行程，多一輪／多 token／多不確定，兩種寫法都要留）；**編譯器只做一件事——判斷哪些子資料夾可安全壓成 inst 鏈**，壓越多越便宜但越看不見（金錢 vs 人類可理解性）；**薄的 `.aos` ＝未編譯、厚的＝編譯過，今天只有厚的**（`G14` 缺的）；父層那條資料夾層級的短接力棒要留，省成「材料齊了就跑」會變 Make ＝ applicative |
