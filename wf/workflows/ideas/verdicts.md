...
| **inst 鏈是語法糖**（2026-09-03） | **攤平、接力棒、`out/` 中間值都是為了省成本而存在的語法糖，不是本體。** 本體只有兩樣：**原子 inst**（工具／LLM 呼叫，不是資料夾、無法再往下託付）與**開／讀／選**（開子資料夾、讀其 `out/`、決定下一個開誰）；其餘都能改寫成「每步一個子資料夾」。（觀察）**攤平＝內聯**，省掉生行程的那幾輪，成本上不是糖——兩種寫法都要留；**編譯器在這套模型裡只做一件事：判斷哪些子資料夾可安全壓成 inst 鏈**，壓越多越便宜但越看不見（金錢 vs 人類可理解性拉鋸）；**薄的 `.aos` ＝未編譯、厚的＝編譯過，今天只有厚的**（`G14` 缺的東西）。父層那條**資料夾層級的短接力棒要留**——省成「材料齊了就跑」會變 Make ＝ applicative，與「父層點名才開」相反 → [nested-eval-sugar](nested-eval-sugar.md) |
