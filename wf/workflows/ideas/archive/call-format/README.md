...
| [cpu-analogy](cpu-analogy.md) | 第三輪：照真實 CPU 的標準驗這個類比——原子性／封閉 ISA／確定性三樣無解（已認），控制流那樣有解**就是 loop**；daemon 讓世界外溢的答案是 **GPU mailbox 模型**；外層契約、plan 9 的 rename 原子性、沒有停機仍開著 | 想知道「CPU 指令」這個說法撐得住多少、loop 為什麼在 core 裡 |
| [handoff-and-world](handoff-and-world.md) | 第四輪：交接協定的三個**實作缺陷**（`.runi` 不是鎖→同回合可能跑兩次、無 `fsync`、`<pid>` 不唯一）＋世界沒圍牆（**用 git 解**）、沒有回合歷史（**要補在 loop**）、PC 不自己前進（**刻意**）、嚴解析鬆執行（**刻意**）、delta time 從 loop 走回來 | 要碰 handoff／loop 之前 |
| [feedback-and-failure](feedback-and-failure.md) | 第五輪：這個系統能不能察覺自己正在壞掉——投遞失敗**沒有回饋路徑**、失敗永遠不算失敗會**全速無限空轉**、交接協定是**自願的**、`.bad` 是規格不認識的第三種狀況、沒有背壓、沒有 change feed | 要碰 handoff／loop 的錯誤處理之前 |
