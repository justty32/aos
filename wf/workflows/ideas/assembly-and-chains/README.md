...
> **節號 §一–§三十 連續編號、跨檔沿用**（拆資料夾前的 §一–§十三 編號不變），
...
**想一次看懂這整包在講什麼**，跳
[lisp-reconciliation](lisp-reconciliation.md)（§二十七–§三十）的身分對照表：
**源碼是 quote 住的部分，series 是正在被求值的部分，inst 是求值碰到底的 primitive。**
...
| [lisp-reconciliation](lisp-reconciliation.md) | §二十七–§三十（**2026-09-03**，全是觀察）。提問：拿 lisp 對標之後，「一串彙編」與 `.h`／`.c` 變成什麼。觀察：**C 線本來就是 lisp 的形狀**（宏展開＋ANF，不是 C 工具鏈）、**身分對照表**（quote／unquote＝`G07`）、**continuation 是資料**是 C 做不到的那一點、`.h` 可以只是源碼 json 的一個欄 |
