...
| 「**開始做這個階段／這件事**」「先規劃再動手」「給我 spec／plan」 | **build-cycle** | [workflows/build-cycle/README.md](workflows/build-cycle/README.md) |
...
> 目前只開了上面這幾個。需要新的開發類工作流（**refactor**／**investigation**／**tooling**…）時**才**加一列，入口檔從單檔開始長——見 [DEV-GUIDE](DEV-GUIDE.md) 的四級成長軌跡，不要預先建空檔。
