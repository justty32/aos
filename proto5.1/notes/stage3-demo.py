...
def pgrep():
    result = subprocess.run(['pgrep', '-f', 'aos-'], text=True, capture_output=True)
    return {'exit': result.returncode, 'stdout': result.stdout, 'stderr': result.stderr}


def main():
    demo = PROTO / '.build' / ('stage3-demo-%d' % time.time_ns())
    ...
    put(agent / 'info.json', {
        ...
        'engine': {'endpoint': trace['endpoint'], 'model': trace['model'], 'cpu': '../C',
