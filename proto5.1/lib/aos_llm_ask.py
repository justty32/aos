#!/usr/bin/env python3
"""組 agent 的請求 body；HTTP call 只供 llm CPU 使用。CLI 只印 body。"""
import argparse
import http.client
import json
import os
...
__all__ = ["EngineFailed", "AgentError", "RESERVED", "build_request", "call", "main"]
...
class EngineFailed(Exception):
    ...
    跟 `AgentError`（設定讀驗錯誤）分開；CPU 把這類錯誤寫成 ok:false 結果。
    `msg` 是白話，`str(e)` 就是它。
    """
...
def build_request(dir, env=None):
    """由 agent 家或已讀驗的 info 組 body；模型真名由 CPU 補上。"""
    info = dir if isinstance(dir, dict) else aos_agent_info.load(dir, env=env)
    body = {}
    messages = []
    if info["system"] != "":
...
def call(engine, body):
    """把 `body` POST 到 `engine`（CPU models 表解好的設定）的 chat/completions，回
