...
(defer (rm-tree tmp)
  (do
    ...
    (spit (string child "/.aos/inst.json")
          "{\"argv\":[\"/bin/sh\",\"-c\",\"printf ran > child-ran.txt\"]}\n")

    (def s0 (stat prog))
    (check "初始 status 是 pc 0 / n 4" (and (= 0 (s0 :pc)) (= 4 (s0 :n)) (not (s0 :done))))
    (def r1 (run prog))
    ...
    (def s4 (stat prog))
    (check "四步完成後 status 是 done" (and (= 4 (s4 :pc)) (= 4 (s4 :n)) (s4 :done) (nil? (s4 :error))))
    (def r5 (run prog))
    (check "完成後再叫是成功空操作" (and (= 0 (r5 :code)) (= "" (r5 :out))))
