...
- **帳簿的新 `tokens_reasoning` 過不了現有 schema**｜依據：本輪任務第 4 項、`schemas/ledger.schema.json` 的 `additionalProperties:false`｜**卡在哪**：原型照任務把思考 token 拆出來後，帳簿每行會多一個 schema 明令不認的欄位。｜**怎麼繞**：照任務不改 schema，原型先寫 `tokens_reasoning`；spec 隊要在 ledger schema 加這個欄位（可為整數或 null）。｜類別：spec 沒講｜擋路程度：擋路
- **`busy_ticks` 沒有合法欄位可放**｜依據：本輪任務第 5 項、`series.schema.json`／`stopped.schema.json` 的 `additionalProperties:false`｜**卡在哪**：任務要接力棒或停止原因檔多記 `busy_ticks`，兩份 schema 都不准這個頂層欄位。｜**怎麼繞**：原型先在兩份都記，讓執行中與停下後都查得到；spec 隊要決定把它升成正式欄位，或規定放進 `ext`。｜類別：spec 沒講｜擋路程度：擋路
- **重啟後結果不明有兩個 reason 名字**｜依據：本輪任務第 2 項、S-09-67｜**卡在哪**：本輪指定狀態檔 reason 是 `unknown_after_restart`，spec 指定的是 `result_unknown`；同一件事會有兩套機器字彙。｜**怎麼繞**：原型狀態檔照本輪任務寫 `unknown_after_restart`，帳簿 outcome 仍用 schema 認得的 `result_unknown`。｜類別：spec 沒講｜擋路程度：煩
- **`requests/` 的內容形狀互相打架**｜依據：本輪任務第 2 項、S-09-63～S-09-64、S-09-89｜**卡在哪**：本輪要求用 rename 把投遞物從 inflight 搬進 requests；spec 則要求 requests 裡是另一種帶 `state`、`unit`、`request` 的狀態物件，原件只能包在 `request` 欄，兩者不能同時靠一次 rename 做到。｜**怎麼繞**：原型照本輪任務保留原件並直接搬檔；spec 隊要裁「原件目錄」與「狀態目錄」是否拆開，或取消原件一字不動 rename 的要求。｜類別：spec 沒講｜擋路程度：擋路
- **任務指定的 unittest 指令在 Python 3.14 收集失敗**｜依據：本輪硬規則 `python3 -m unittest discover proto/tests`｜**卡在哪**：這種跑法把 `test_*.py` 當頂層模組匯入，原有的 `from .helpers` 沒有 parent package，9 支測試在收集階段全倒；`run-all.sh` 用 `-s proto/tests -t .` 則能跑。｜**怎麼繞**：允許範圍內的測試檔改成同時支援 package 與頂層匯入，兩種指令現在都會跑同一批。｜類別：spec 沒講｜擋路程度：煩
- **改名後的真模型收檔腳本還指著 `llm-done/`**｜依據：本輪「只准碰」清單與第 2 項「更新所有引用」｜**卡在哪**：`proto/play-agent.sh` 第 193 行會收舊的 `llm-done/`，但本輪只特許改這支檔的 `max_wait_ms` 那一行；直接改收檔行會越過明定範圍。｜**怎麼繞**：本輪不碰那一行；下輪若開放 `play-agent.sh` 的收檔段，應改收 `.aos/requests/`，不然真模型紀錄會少請求原件。｜類別：多餘的動作｜擋路程度：煩

## 指揮一隊原型的發現

- **daemon 起的 run 把非零結束碼看成 0（T-01）**｜依據：S-08-06、S-08-31、S-10-30、I-04｜**卡在哪**：daemon 的 `SIGCHLD=SIG_IGN` 跨 exec 傳給 run，讓 Python `waitpid` 拿到 ECHILD，指令失敗靜默變成功。｜**怎麼繞**：第 2 輪已改成子端恢復 `SIG_DFL`、daemon 用 `waitpid(WNOHANG)` 收屍。｜類別：看不見的狀態｜擋路程度：擋路
- **父分不出子做完、壞掉、被停（T-02）**｜依據：S-07-14、S-07-18、S-07-29、S-08-30｜**卡在哪**：四種結局在登記表都是 `stopped`／`pid:null`，漏寫落點時父只會一直等。｜**怎麼繞**：第 2 輪讓 daemon 對帳讀子地停止原因並代寫 `no_result`／`child_failed`／`killed` 狀態檔。｜類別：看不見的狀態｜擋路程度：擋路
- **兩個子共用落點會互蓋（T-03）**｜依據：S-07-60～S-07-63、S-07-78｜**卡在哪**：原型原先沒查同格重複落點，也不查結果、狀態、用量三檔是否已有舊檔。｜**怎麼繞**：第 2 輪加整格預檢，違反就以解析錯誤 3 拒跑。｜類別：看不見的狀態｜擋路程度：擋路
- **落點沒清就重跑會吃到舊結果**｜依據：S-07-60～S-07-62｜**卡在哪**：主可能在新子尚未做完前讀到上一輪檔案。｜**怎麼繞**：第 2 輪已拒絕三個落點檔任一已存在；人要先清或換路徑。｜類別：看不見的狀態｜擋路程度：擋路
- **重跑一隊要清與還原很多地方**｜依據：S-07-62｜**卡在哪**：三塊地的機器狀態、落點、agent state、報告與被改原稿都要分開收。｜**怎麼繞**：`play-team.sh` 集中清理並從 `.pristine/` 還原。｜類別：多餘的動作｜擋路程度：煩
- **子能靠父路徑讀寫落點外的檔**｜依據：S-07-08、S-07-64、S-07-65｜**卡在哪**：目前只有自律，沒有機制限制子只寫明示落點。｜**怎麼繞**：劇本約定子只讀材料、只寫 `AOS_RESULT`；正式隔離仍待做。｜類別：spec 沒講｜擋路程度：擋路
- **派 N 個子與等 N 個子都要 N 步，且只能排隊等**｜依據：05 接力棒、S-07-07｜**卡在哪**：沒有等一組或等最先完成者的步，前一個 await 會遮住後一個已失敗的消息。｜**怎麼繞**：用較小 `max_ticks` 當保險絲，報告步再查全部落點三態。｜類別：spec 沒講｜擋路程度：擋路
- **await 計數跨全停重開不歸零**｜依據：S-07-22、S-10-33｜**卡在哪**：重開後會沿用舊接力棒裡的等待格數，分不出工作慢或中間停機。｜**怎麼繞**：劇本把 `max_ticks` 開大；正式語意仍要定。｜類別：spec 沒講｜擋路程度：擋路
- **指令面沒有只查一塊子地的鐘**｜依據：S-07-29、S-08-30、S-08-51｜**卡在哪**：`daemon ls <地>` 不過濾，agent 得自己解析整張 JSON。｜**怎麼繞**：劇本只靠落點三態。｜類別：錯誤不指路｜擋路程度：煩
- **手動跑子地拿不到派活環境**｜依據：S-07-12、S-07-13、08b｜**卡在哪**：`AOS_RESULT`／`AOS_CALLER`／`AOS_ARG_*` 只在 daemon 依登記表起 run 時才有。｜**怎麼繞**：worker 自己檢查缺少的變數並提示要由父派活。｜類別：錯誤不指路｜擋路程度：煩
- **一次全停漏掉 LLM 世界**｜依據：S-08-84、F-02｜**卡在哪**：原型沒把 `llm-serve` runner 登記進 daemon，所以 daemon stop 只停三塊 agent 地。｜**怎麼繞**：劇本另記 serve pid 並在收尾補停。｜類別：看不見的狀態｜擋路程度：擋路
- **全停後重開不會自動續跑**｜依據：S-08-85、S-10-33｜**卡在哪**：原型沒有 `resume`，daemon 只起 `pending`。｜**怎麼繞**：逐地再做一次 `daemon add`。｜類別：多餘的動作｜擋路程度：擋路
- **看一隊現況要敲 1+N 次**｜依據：12 指令面、S-08-51｜**卡在哪**：daemon ls 只講鐘，status 又不講 await 的落點與已等格數。｜**怎麼繞**：劇本定時合併列出登記表與三塊地狀態。｜類別：看不見的狀態｜擋路程度：擋路
- **兩個子只差設定仍要複製整份腦**｜依據：S-10-01、03 地的隔離｜**卡在哪**：沒有可共用的 agent 模板地。｜**怎麼繞**：複製 worker 目錄，再只改 `agent.json`。｜類別：多餘的動作｜擋路程度：煩
- **起隊伍缺少一條完整初始化命令**｜依據：S-08-86、S-10-07｜**卡在哪**：三次 init、三份 path 設定、LLM serve、daemon 與主鐘要分開做。｜**怎麼繞**：全部包進 `play-team.sh`。｜類別：多餘的動作｜擋路程度：煩
- **agent 限制欄位目前只是裝飾**｜依據：S-10-08～S-10-11｜**卡在哪**：原型不讀 `agent.json` 的上限，也不累加 `.aos/usage.json`。｜**怎麼繞**：agent 腦另讀自訂 `max_rounds`。｜類別：spec 沒講｜擋路程度：小
- **echo 假後端沒有真正模型答案**｜依據：09 假後端｜**卡在哪**：要測完整流程，只能先把備用答案種進 prompt 再自行抽行。｜**怎麼繞**：worker／lead 各自實作挑 `DOC:`／摘要行。｜類別：spec 沒講｜擋路程度：小

原文見 `proto/examples/team/FINDINGS-team.md`。

## 門房第一級原型的發現

- **本子檔名與欄名有兩套**｜依據：S-13-11～S-13-13 與 doorman 任務介面｜**卡在哪**：spec 要 `doorman.log`／`event` 且 schema 禁多餘欄，原型任務要 `doorman.jsonl`／`kind`。｜**怎麼繞**：原型用 `.jsonl` 並同寫 `kind`、`event`。｜類別：spec 沒講｜擋路程度：煩
- **只認 layout 原子改名有監看競態**｜依據：S-13-47｜**卡在哪**：`.aos/` 與 layout 幾毫秒內建完，門房常來不及先裝 watch，出生完全漏掉。｜**怎麼繞**：預設掃描也認，`--strict-birth` 才重現嚴格行為。｜類別：看不見的狀態｜擋路程度：擋路
- **init 不寫 land_id，嚴格門房會拒絕所有地**｜依據：S-13-49、layout schema｜**卡在哪**：現有原型 layout 沒 required 的 `land_id`。｜**怎麼繞**：預設只警告並登記 null，`--strict-layout` 才拒絕。｜類別：錯誤不指路｜擋路程度：擋路
- **孤魂補刀的訊號、範圍與時限沒定**｜依據：S-13-17、S-08-77｜**卡在哪**：地已死，不能走控制收件匣，也沒規定殺 pid 還是群組。｜**怎麼繞**：原型對行程群組先 TERM 0.5 秒，再 KILL 0.5 秒。｜類別：spec 沒講｜擋路程度：擋路
- **門房補刀與狀態檔代寫的責任衝突**｜依據：S-08-48、S-08-78、S-13-08、S-13-09｜**卡在哪**：門房不能改任何地，但父又需要 killed 狀態才能結束 await。｜**怎麼繞**：第 2 輪讓門房只記 `needs_status`，daemon 對帳代寫。｜類別：看不見的狀態｜擋路程度：擋路
- **死亡欄位沒有正式位置且可能被 daemon 刪掉**｜依據：registry schema、S-08-36～S-08-37、S-13-18｜**卡在哪**：`died_at` 等只能塞 ext；門房留墓碑、daemon 刪死地兩種規則互撞。｜**怎麼繞**：原型暫留 `ext.died_at`／`died_by`／`killed_pid`／`reborn_at`。｜類別：看不見的狀態｜擋路程度：擋路
- **監看範圍有雞生蛋問題**｜依據：S-13-30、S-13-31｜**卡在哪**：深處的地要先登記才會被看，但又得先被看才會自動登記；spec 也沒定義監看根。｜**怎麼繞**：原型加 `root` 與 `--depth`，預設只掃兩層。｜類別：多餘的動作｜擋路程度：煩
- **stopped 同時表示四種不同狀態**｜依據：S-08-29、S-13-18｜**卡在哪**：正常做完、使用者停、剛出生、地已死在 daemon ls 都長一樣。｜**怎麼繞**：死亡細節暫放 ext；畫面仍未完整呈現。｜類別：看不見的狀態｜擋路程度：煩
- **本子沒有游標與輪替**｜依據：S-13-51～S-13-52｜**卡在哪**：daemon 若每五秒從頭讀，事件越多成本越高，也沒有可靠位置記讀到哪。｜**怎麼繞**：第一級先接受整份讀；做 write 記帳前必須補契約。｜類別：多餘的動作｜擋路程度：小
- **退回輪詢後何時回 inotify 沒講**｜依據：S-13-32～S-13-33｜**卡在哪**：缺重試節奏與 fallback 重複記錄規則。｜**怎麼繞**：原型切換時只記一次，之後維持輪詢直到退出。｜類別：spec 沒講｜擋路程度：小

原文見 `proto/DOORMAN.md`。
