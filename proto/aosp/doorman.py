#!/usr/bin/env python3
"""門房第一級（spec 13）——`aos doorman` 的核心，只看不擋。
...
沒有發明第二種鎖。`proto/doorman.py` 只是一行相容薄殼，核心都在本模組。
...
class Doorman:
    ...
    def _death(self, land, trigger):
        ...
        if killed is not None:
            needs_status = e.get("result") if e else None
            detail = {"trigger": "reap", "killed_pid": killed}
            if needs_status:
                detail["needs_status"] = needs_status
            self.log(GONE, land, detail)
...
def run_args(a):
    """執行已由外層 argparse 解析好的門房參數，供 `aos doorman` 共用。"""
    if a.tmpfs_note:
    ...
    if not a.root:
        sys.stderr.write("錯誤：沒給根目錄\n下一步：aos doorman <根目錄> [--home <AOS_HOME>]\n")
        return 2
