...
- **`inst_t::stderr_path` 不能再只是一個字串**：它要變成三態
  「繼承／檔案路徑／併入 stdout」。這是公開結構的改動，C ABI 要跟著補。
...
**待拍板 ①：相對於誰？** **建議相對於 instruction 檔自己所在的目錄**（也就是
`.aos/`），這樣整個資料夾可以連 `.aos` 一起搬走而不失效——這正好呼應回合模型「資料夾
就是世界」。

**建議直接照抄 freepy v1 的拒絕清單**：`http(s):`、`file:`、絕對路徑、以及用 `..` 逃出
root 的相對路徑，**全部拒絕**。

**待拍板 ②：深度。** **建議 v1 深度 1，不遞迴**——被 `$ref` 進來的內容裡如果還有
`$ref`，不再展開。理由和 freepy 一樣：一個小檔可能連到整棵樹，不該一次呼叫就無上限
展開；順帶也不用處理循環。
...
**建議 B**。`format` 產出的 `inst_t` 帶著未解析的指示詞，`resolve(inst_t&, ctx)` 把它
變成可執行的 `inst_t`，`exec` 完全不變。這也讓「解析的環境從哪來」變成 `ctx` 的一個
明確參數，而不是藏在某層裡的隱含行為。

## 七、對現有契約的影響

- **round trip 變成有損**。`write_one(read_one(doc))` 會吐出**已解析**的值，不是原本的
  指示詞。現有那個 round-trip 測試（[`test_format_write.cpp`](../core/inst/tests/test_format_write.cpp)）
  用的是純字串，所以照樣會綠——但這件事要明白寫進格式文件，不要讓人以為 round trip 是
  無損的。
- **要新增錯誤狀態**：不認得的 `$xxx`、指示詞物件有多於一個鍵、指示詞的值不是字串、
  環境變數不存在、`$ref` 目標不存在或逃出 root、`$opt` 用在不合法的欄位。
- **C ABI 要跟**：`stderr` 從單純字串變成三態，getter／setter 要重新設計。
- **安全面變寬**。instruction 檔本來就等同可執行程式碼（見
  [`core/inst/docs/exec.md`](../core/inst/docs/exec.md) 的安全性一節），但 `$ref` 讓它
  多出「讀任意檔案」這個面——把讀到的內容塞進 argv 就是一條外流路徑。**拒絕絕對路徑與
  `..` 逃逸不是防禦深度，是必要條件。**
...
1. **`$opt` + `stderr` merge** — 最小的一步，而且直接把 [D5](roadmap.md#d5) 解掉。它不
   需要讀環境也不需要讀檔，所以**連第六節那個分層問題都不會碰到**（`merge` 是純語法，
   `format` 自己就能決定）。
2. **`$env`** — 開始需要解析上下文，第六節的分層決定在這一步必須先有答案。
3. **`$ref`** — 最大的一步：檔案系統、路徑安全、JSON Pointer、深度限制。
