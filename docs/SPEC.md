...
- **§D-4 彙整規則**：只收**沒有狀況後綴**的投遞。順序不保證——投遞者 **MUST NOT**
  ...
  除此之外 **MUST NOT** 刪。
  **發布之前，彙整 MUST 再確認本輪收下的投遞都還在**；有任何一份已經不在，就 **MUST**
  放棄本輪——不發布、不清投遞、不寫 header。批的**來源必須在提交點當下仍然成立**：
  投遞不見了代表另一個彙整者已經把同一組發布並清理完，此時再發布一次就是同一批執行
  兩次。這**不是**互斥（`.runi` 不是鎖那條仍開著），只把窗口縮到查核與 rename 之間。
  來源：aos-folder 六；「本輪處理完成」的三個例外照實寫明，裁於 2026-08-28（M1 審查修補 #16）；
  發布前的來源查核裁於 2026-08-28（M1 審查修補複測：`swept` 閘門打開後，原本靠陳舊
  header 意外擋住的併發雙重彙整暴露出來，重複執行率由 8.7% 升到 35%，加上本查核後回到 0–1%）。
...
- **§D-6 批 id 與去重**：批 `id` ＝ 對排序後（投遞檔名＋內容）的
  ...
  **覆蓋範圍**：只保證整組殘留；部分殘留混入新投遞後的重複不在保證內（條款照實寫，
  ...
  那個方向由 §D-5 的**排他發布**與 §D-4 的**發布前來源查核**一起擋——排他發布只在
  勝出者那份批**還沒被取走**時有效（取件把 `inst.json` 換成 `.runi`，槽位就空了），
  所以真正涵蓋整段窗口的是來源查核。
