    ...
    (loop while (and (< n (w-get llmw :max-per-tick)) (w-get llmw :queue))
          do (let ((req (pop (gethash :queue llmw))))
               ...
               (multiple-value-bind (reply err)
                 ...
                 (let ((asker (world-at (getf req :from))))
                   (when asker
                     (send-mail llmw asker
...
(defun make-llm (name engine &optional (max-per-tick 1))
  ...
  (make-world name :engine engine :queue '() :usage (make-hash-table :test 'equal)
                   :max-per-tick max-per-tick :seq 0 :func-tick #'llm-tick))
