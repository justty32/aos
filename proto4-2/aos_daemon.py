...
daemon 請求只有三個 op：
    {"op":"spawn","name":…,"dir":…,"interval":…}   開一顆 cpu 去跑那個資料夾
    {"op":"kill","name":…}                          SIGTERM 那顆 cpu
    {"op":"ls"}                                     回登記表
...
class Daemon:
    def __init__(self, home):
        self.home = home
        self.cpus = {}          # name -> {"proc": Popen, "pid", "dir", "interval"}
        self.reaping = []       # 已經 SIGTERM、還在等它斷氣的（不收就變殭屍）
        self.stopping = False
        self.log = None

    # ── cpu ────────────────────────────────
    def spawn(self, name, d, interval):
        d = os.path.abspath(d)
        if name in self.cpus:               # 同名先收掉舊的
            self.kill(name)
        p = subprocess.Popen(
        ...
        self.cpus[name] = {"proc": p, "pid": p.pid, "dir": d, "interval": interval}
        self.say("spawn %s pid=%d dir=%s 每 %s 秒" % (name, p.pid, d, interval))
        return {"name": name, "pid": p.pid, "dir": d, "interval": interval}

    def kill(self, name):
        c = self.cpus.pop(name, None)
        if not c:
            raise KeyError(name)
        try:
            c["proc"].terminate()
        ...
        self.say("kill %s pid=%d" % (name, c["pid"]))
        return {"name": name, "pid": c["pid"]}

    def ls(self):
        return self.table()

    def table(self):
        """每顆 cpu 現在長怎樣：pid 活不活、跑到第幾格、佔多少記憶體。"""
        t = {}
        for name, c in self.cpus.items():
            c["proc"].poll()                     # 順手收殭屍
            a = c["proc"].returncode is None
            t[name] = {"pid": c["pid"], "dir": c["dir"], "interval": c["interval"],
    ...
    def handle_requests(self):
        ...
        for n in names:
            ...
            try:
                ...
                op = req["op"]
                if op == "spawn":
                    r = self.spawn(req["name"], req["dir"], req.get("interval", 1))
                elif op == "kill":
                    r = self.kill(req["name"])
                elif op == "ls":
                    r = self.ls()
                else:
                    raise KeyError(op)
    ...
    def save(self):
        t = self.table()
        write_json(self.home.cpusf, {n: {k: c[k] for k in ("pid", "dir", "interval")}
                                     for n, c in self.cpus.items()})
    ...
    def serve(self):
        ...
        self.spawn(KERNEL_NAME, self.home.kernel, KERNEL_INTERVAL)   # 最初的那顆 cpu
        try:
