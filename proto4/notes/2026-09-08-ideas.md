...
- **`--home` 可省略**：`--home H` → `AOS_HOME` → 預設 `~/.aos-daemon`（使用者：「預設 ~/.aos-daemon 就好，--home 可省略」）。家＝daemon 與 ctl 靠檔案講話的那個資料夾（requests/、done/、daemon.pid、state.json、daemon.log），沒有 socket、沒有 port。
