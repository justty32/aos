...
`aos-run xxx [aos-exec 的旗標] [--interval SEC] [--from-start] [--max-runs N] [--time-limit SEC] [--stop-exit CODE]`

- **aos-exec 有的旗標它都有**：`--dir-target`、`--timeout-ms`（每一次執行的上限）。核心就是反覆呼叫 proto4-3 的 `run_target()`，不重寫。
- **`--interval SEC`**（預設 1）：兩次執行之間的間隔。
...
- **停止條件**（任一成立就停）：
  ...
  - `--time-limit SEC`：從 aos-run 起跑算的整體時限，**硬時限**：正在睡→醒來就退；正在跑→那次被砍（做法：呼叫 `run_target` 時把 `timeout_ms` 換成 min(原本的, 剩餘時間)，不用另開執行緒）。
