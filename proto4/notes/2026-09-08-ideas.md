...
**第四種指示詞 `$fmt`**（使用者補的：「envs 這塊，剛剛那個 PATH，得要用 `"PATH":{"$fmt":...}`」）——用來接字串。`{"$fmt":"$PATH:/opt/bin"}`：字串裡的 `$NAME`／`${NAME}` 換成 aos-exec 自己的環境變數（不是 inst.json 的 `envs`），`$$`＝一個字面的 `$`，`$` 後面接不合法名字＝格式錯誤。變數不存在＝錯誤、空＝空字串。能寫 `$env` 的地方都能寫 `$fmt`。這是我定的細節，使用者只給了 `$fmt` 這個名字和 `$PATH:ahah` 這個例子。
